    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                domain = utils.get_domain_from_entity(utils.get_entity_original_identifier(entity)) if \
                    entity.entity_type == EntityTypes.URL else utils.get_entity_original_identifier(entity)
                siemplify.LOGGER.info("Fetching information for domain {}".format(domain))
                ...
                siemplify.LOGGER.info("Found information about {}. Highest source severity: {}".format(
                    domain, ioc_details.highest_source_severity[0]))

                entity.additional_properties.update(ioc_details.as_enrichment(prefix=ENRICHMENT_PREFIX, for_domain=True))
                ...
                if create_insight and not only_suspicious_insight:
                    siemplify.add_entity_insight(entity, ioc_details.to_insight(for_domain=True))
                elif create_insight and only_suspicious_insight and is_suspicious:
                    siemplify.add_entity_insight(entity, ioc_details.to_insight(for_domain=True))

                siemplify.result.add_entity_table(entity.identifier, construct_csv(ioc_details.to_table(for_domain=True)))
        ...
        if successful_entities:
            output_message += "Successfully enriched the following domains from Google Chronicle:\n   {}".format(
        ...
        if failed_entities:
            output_message += "\n\nAction was not able to enrich the following URLs from Google Chronicle:\n   {}".format(
