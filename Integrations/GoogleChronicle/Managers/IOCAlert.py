...
IDS_FILE_NAME = "IOC_ids.json"
IDS_DB_KEY = "IOC_ids"
TIMESTAMP_FILE_NAME = "IOC_timestamp.stmp"
TIMESTAMP_DB_KEY = "IOC_timestamp"
TIMESTAMP_KEY = "last_seen_time_ms"
...
class IOCAlert:
    ...
    def read_ids(self):
        ...
        self.siemplify.LOGGER.info(f"Successfully loaded {len(existing_ids)} existing "
                                   f"{ALERT_TYPE_NAMES.get(ALERT_TYPES.get('ioc'))} ids")
        return existing_ids
    ...
    def get_alerts(self, existing_ids, fetch_limit, hours_backwards, fallback_severity=None, padding_period=None):
        ...
        :return: {list} list of IOC objects
        """
        ...
        if padding_period and last_success_timestamp > unix_now() - convert_hours_to_milliseconds(padding_period):
            ...
            self.siemplify.LOGGER.info(f"Last success time is greater than {ALERT_TYPE_NAMES.get(ALERT_TYPES.get('ioc'))} "
                                       f"alerts padding period. Unix: {last_success_timestamp} will be used as "
                                       f"last success time")

        is_more_data, alerts = self.manager.list_iocs(
        ...
        self.siemplify.LOGGER.info(f"Fetched {len(filtered_alerts)} {ALERT_TYPE_NAMES.get(ALERT_TYPES.get('ioc'))} "
                                   f"alerts")
        return filtered_alerts

    def pass_filters(self, alert):
        filters = get_filters_by_alert_type(self.siemplify.LOGGER, self.siemplify.whitelist, ALERT_TYPES.get("ioc"))
    ...
    def save_timestamp(self, alerts):
        ...
        :param alerts: {list} list of IOC objects
        :return: {void}
        """
