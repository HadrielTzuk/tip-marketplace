...
def main():
    ...
    for entity in siemplify.target_entities:
        try:
            ...
            if device:
                device_id = device["id"]
                ports = portnox_manager.get_device_open_ports(device_id)

                if ports:
                    csv_output = construct_csv(ports)
    ...
    if enriched_entities:
        ...
        output_message = 'Open ports were found for the following entities:\n' + '\n'.join(
            entities_names)

        siemplify.update_entities(enriched_entities)

    else:
        output_message = 'No open ports were found.'
