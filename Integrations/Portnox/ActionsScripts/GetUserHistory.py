...
SCRIPT_NAME = "Portnox - GetUserHistory"
...
def main():
    ...
    for entity in siemplify.target_entities:
        try:
            ...
            if device:
                device_id = device["id"]
                user_history = portnox_manager.get_device_user_history(device_id)

                if user_history:
                    csv_output = construct_csv(user_history)
    ...
    if enriched_entities:
        ...
        output_message = 'User history was found for the following entities:\n' + '\n'.join(
            entities_names)

        siemplify.update_entities(enriched_entities)

    else:
        output_message = 'No user history was found.'
