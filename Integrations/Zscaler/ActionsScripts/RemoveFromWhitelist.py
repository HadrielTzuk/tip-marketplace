...
from ZscalerManager import ZscalerManager, REMOVE_WHITE_LIST_KEY, ZscalerMissingError
...
def main():
    ...
    siemplify.script_name = 'Zscaler - Remove from whitelist'
    ...
    for entity in siemplify.target_entities:
        ...
        if entity_to_block:
            try:
                zscaler_manager.update_to_whitelist(entity_to_block, action=REMOVE_WHITE_LIST_KEY)
                # apply changes
                ...
                siemplify.LOGGER.info(
                    "Unnecessary operation. {0} already removed from the whitelist".format(entity.identifier))
    ...
    if domains:
        output_message += 'Successfully remove the following entities from the Urls whitelist:\n' +\
                         '\n'.join(domains)
    ...
    if already_exists:
        output_message += '\n The following entities already removed from the whitelist:\n' + \
                          '\n'.join(already_exists)

    if not errors and not domains and not already_exists:
        output_message = "No entities were removed from the Urls whitelist"
        result_value = 'false'
