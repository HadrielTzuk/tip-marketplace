...
def main():
    ...
    siemplify.script_name = 'Zscaler - Add to blacklist'
    ...
    for entity in siemplify.target_entities:
        ...
        if entity_to_block:
            try:
                zscaler_manager.add_to_blacklist(entity_to_block)
                # apply changes
    ...
    if domains:
        output_message += 'Added the following entities to the Urls blacklist successfully:\n' +\
                         '\n'.join(domains)

    if errors:
        output_message += '\n Errors occurred, check log for more information'

    if not errors and not domains:
        output_message = "No entities were added to the Urls blacklist"
        result_value = 'false'
