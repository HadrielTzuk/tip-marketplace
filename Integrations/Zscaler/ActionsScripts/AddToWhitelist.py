...
from ZscalerManager import ZscalerManager, ADD_WHITE_LIST_KEY, ZscalerMissingError
...
def main():
    ...
    siemplify.script_name = 'Zscaler - Add to whitelist'
    ...
    for entity in siemplify.target_entities:
        ...
        if entity_to_block:
            try:
                zscaler_manager.update_to_whitelist(entity_to_block, action=ADD_WHITE_LIST_KEY)
                # apply changes
                ...
                siemplify.LOGGER.info(
                    "Unnecessary operation. The whitelist already contains {0}".format(entity.identifier))
    ...
    if domains:
        output_message += 'Added the following entities to the Urls whitelist successfully:\n' + \
                          '\n'.join(domains)
    ...
    if already_exists:
        output_message += '\n The following entities already exists in the whitelist:\n' + \
                          '\n'.join(already_exists)

    if not errors and not domains and not already_exists:
        output_message = "No entities were added to the Urls whitelist"
        result_value = 'false'
