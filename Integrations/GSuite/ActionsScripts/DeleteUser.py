...
def main():
    ...
    siemplify.script_name = "{} - {}".format(INTEGRATION_NAME, DELETE_USER_SCRIPT_NAME)
    ...
    email_address = extract_action_param(siemplify, param_name="Email Address", is_mandatory=True)
    ...
    try:
        ...
        gsuite_manager.delete_user(email_address)
        output_message = f"Successfully deleted user {email_address}"
        status = EXECUTION_STATE_COMPLETED
        result_value = True
    except Exception as error:
        output_message = f"Error executing action {DELETE_USER_SCRIPT_NAME}. Reason: " \
                         f"{'User not found.' if isinstance(error, GSuiteNotFoundException) else f'{error}'}"
