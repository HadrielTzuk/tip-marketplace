...
def main():
    ...
    siemplify.script_name = "{} - {}".format(INTEGRATION_NAME, DELETE_OU_SCRIPT_NAME)
    ...
    ou_path = extract_action_param(siemplify, param_name="OU Path", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        gsuite_manager.delete_ou(
            customer_id=customer_id,
            path=ou_path
        )
        output_message = f"Successfully deleted organization unit for customer id {customer_id} with organization unit path {ou_path}"
    except Exception as error:
        output_message = f"Error executing action {DELETE_OU_SCRIPT_NAME}. Reason: " \
                         f"{'Customer ID is invalid' if isinstance(error, GSuiteValidationException) else f'{error}'}"
