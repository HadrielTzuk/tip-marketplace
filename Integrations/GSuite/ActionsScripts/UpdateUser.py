...
def main():
    ...
    siemplify.script_name = "{} - {}".format(INTEGRATION_NAME, UPDATE_USER_SCRIPT_NAME)
    ...
    given_name = extract_action_param(siemplify, param_name="Given Name", is_mandatory=False, print_value=True)
    family_name = extract_action_param(siemplify, param_name="Family Name", is_mandatory=False, print_value=True)
    password = extract_action_param(siemplify, param_name="Password", is_mandatory=False, print_value=False)
    ...
    change_password_at_next_login = extract_action_param(siemplify, param_name="Change Password At Next Login", input_type=bool,
                                                         is_mandatory=True,
                                                         default_value=False,
                                                         print_value=True)
    user_status = extract_action_param(
        siemplify,
        param_name="User Status",
        print_value=True
    )
    suspended = USER_STATUS_TO_SUSPENDED[user_status]
    ...
    try:
        ...
        user = gsuite_manager.update_user(
        ...
        output_message = f"Successfully updated user with primary email address {primary_email}"
    except Exception as error:
        output_message = f'Error executing action {UPDATE_USER_SCRIPT_NAME}. Reason: {error}'
