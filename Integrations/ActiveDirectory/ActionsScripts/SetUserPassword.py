...
SCRIPT_NAME = "ActiveDirectory - SetUserPassword"
...
def main():
    ...
    new_password = extract_action_param(siemplify, param_name="New Password", is_mandatory=True, input_type=str)
        ...
        if target_entities:
            for entity in target_entities:
                ...
                if unix_now() >= siemplify.execution_deadline_unix_time_ms:
                    ...
                    break
                try:
                    if manager.set_user_password(entity.identifier, new_password):
            ...
            if successful_entities:
                output_message += "Active Directory - Set password for following user:\n   {} \n".format(
                    "\n   ".join([entity.identifier for entity in successful_entities])
                )
                result_value = True
            if failed_entities:
                if incorrect_password:
                    output_message += "Failed to execute action 'Set User Password' - either you don't have an SSL " \
                                      "connection established with Active directory, or the provided password wasn't " \
                                      "strong enough according to Active Directory, please check the connection and " \
                                      "try again"
