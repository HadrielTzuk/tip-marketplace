...
ACTION_NAME = "SymantecATP_Isolate Endpoint"
INSIGHT_MESSAGE = "Isolate command was sent."
...
def main():
    ...
    for entity in target_entities:
        try:
            ...
            if entity_uuid:
                command_id = atp_manager.isolate_endpoint_by_uid(entity_uuid)

            if command_id:
                command_ids_csv.append({"Entity Identifier": entity.identifier, "Command ID": command_id})
                siemplify.add_entity_insight(entity, INSIGHT_MESSAGE, triggered_by=ATP_PROVIDER)
                isolated_entities.append(entity)
                command_ids.append(command_id)

        except Exception as err:
            error_message = 'Error isolating "{0}", Error: {1}'.format(entity.identifier, unicode(err))
            siemplify.LOGGER.error(error_message)
            siemplify.LOGGER.exception(err)
            errors.append(error_message)

    if command_ids_csv:
        output_message = "{0} isolated successfully.".format(','.join([entity.identifier for entity in
                                                                       isolated_entities]))
