...
def main(is_test_run):
    ...
    lowest_risk_to_fetch = extract_connector_param(
        siemplify,
        param_name="Lowest Risk Level To Fetch",
        print_value=True
    )
    ...
    whitelist_as_a_blacklist = extract_connector_param(
        siemplify, "Use whitelist as a blacklist",
        is_mandatory=True,
        input_type=bool,
        print_value=True
    )
    ...
    try:
        ...
        if fetch_limit < 1:
            siemplify.LOGGER.info(
                f"Max Alerts To Fetch must be greater than zero. "
        ...
        if lowest_risk_to_fetch and lowest_risk_to_fetch.lower() not in SEVERITIES:
            raise Exception(
                f"Invalid value given for Lowest Risk Level To Fetch parameter. "
        ...
        filtered_alerts = manager.get_risk_detections(
            existing_ids=existing_ids,
            limit=fetch_limit,
            start_timestamp=last_success_time,
            lowest_severity=lowest_risk_to_fetch
        )
        ...
        for alert in filtered_alerts:
            try:
                ...
                if len(processed_alerts) >= fetch_limit:
                    ...
                    siemplify.LOGGER.info(
                        "Reached max number of alerts cycle. No more "
                        "alerts will be processed in this cycle."
                    )
                    break
                ...
                if not pass_filters(siemplify, whitelist_as_a_blacklist, alert, "risk_event_type"):
                    continue
        ...
        if not is_test_run:
            ...
            save_timestamp(
                ...
                timestamp_key="detected_date_time",
