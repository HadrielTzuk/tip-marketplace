    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                siemplify.LOGGER.info('Successfully removed tag {} from {}'.format(tag_name, entity.identifier))
        ...
        if successful_entities:
            output_message += 'Successfully removed tag {} from the following endpoints from McAfee Mvision ePO V2: \n{}'.format(
        ...
        if failed_entities:
            output_message += '\n\nAction was not able to removed tag from the following endpoints from McAfee Mvision ePO V2: \n{}\n'.format(
                '\n'.join(failed_entities))

    except TagNotFoundException:
        result_value = False
        output_message = "Action wasn't able to remove tag '{0}'. Reason: Tag '{0}' was not found in McAfee Mvision ePO. " \
        ...
        output_message = "Error executing action '{}'. Reason: {}".format(REMOVE_TAG_SCRIPT_NAME, e)
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
