...
from constants import PROVIDER_NAME, START_SUBSCRIPTION_SCRIPT_NAME


@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = START_SUBSCRIPTION_SCRIPT_NAME
    ...
    # Action Parameters
    subscription_type = extract_action_param(siemplify, default_value="Select content type",
                                             param_name="Start a Subscription for", is_mandatory=True, input_type=str)
    ...
    try:
        office365_manager = Office365ManagementAPIManager(api_root=api_root,
        ...
        office365_manager.start_subscription(subscription_type)
        
        output_message = "Successfully created O365 Management API subscription for the {} content type!".format(subscription_type)
    except Exception as e:
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = "Failed to execute command! Error is {}".format(e)
