...
from constants import CREATE_BACKGROUND_SCAN_TASK_SCRIPT_NAME, INTEGRATION_NAME, NEW_LINE, PROVIDER_NAME
...
def main():
    ...
    siemplify.script_name = CREATE_BACKGROUND_SCAN_TASK_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                device = devices[-1]
                siemplify.LOGGER.info(
                    f"Creating background scan task for device: {device.id}, entity:{entity_identifier}.")

                manager.create_background_scan_task(device_id=device.id)
        ...
        if successful_entities:
            output_message += f"Successfully created a background scan task for the following entities:\n " \
        ...
        if failed_entities:
            output_message += f"Failed to create background scan tasks for the following entities:\n " \
                              f"{NEW_LINE.join(failed_entities)}"

    except Exception as e:
        output_message = f"Error executing action {CREATE_BACKGROUND_SCAN_TASK_SCRIPT_NAME}. Reason: {e}"
