...
from constants import INTEGRATION_NAME, CREATE_DISABLE_BYPASS_MODE_SCRIPT_NAME, PROVIDER_NAME
...
def main():
    ...
    siemplify.script_name = CREATE_DISABLE_BYPASS_MODE_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                siemplify.LOGGER.info(
                    f'Creating disable bypass mode task for device: {device.id}, entity:{entity_identifier}.')
                manager.create_disable_bypass_mode_task(device.id)
        ...
        if successful_entities:
            output_message += f"Successfully created an disable bypass mode task for the following entities:\n " \
        ...
        if failed_entities:
            output_message += f"Failed to create disable bypass mode tasks for the following entities: " \
                              f"{', '.join(failed_entities)}\n"

    except Exception as e:
        output_message = f'Error executing action {CREATE_DISABLE_BYPASS_MODE_SCRIPT_NAME}. Reason: {e}'
