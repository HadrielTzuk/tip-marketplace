...
from constants import INTEGRATION_NAME, QUARANTINE_DEVICE_SCRIPT_NAME, PROVIDER_NAME, NEW_LINE
...
def main():
    ...
    siemplify.script_name = QUARANTINE_DEVICE_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                if not devices:
                    ...
                    json_results[entity_identifier] = {"status": "failure"}
                ...
                siemplify.LOGGER.info(
                    f'Creating enable quarantine device task for device: {device.id}, entity:{entity_identifier}.')
                manager.create_quarantine_task(device.id)

                successful_entities.append(entity_identifier)
                json_results[entity_identifier] = {"status": "done"}
        ...
        if successful_entities:
            output_message += f'Successfully created a quarantine device task for the following entities:\n ' \
                              f'{NEW_LINE.join(successful_entities)} \n'

            if failed_entities:
                output_message += f'Failed to create quarantine device tasks for the following entities:\n ' \
        ...
        output_message = f'Error executing action {QUARANTINE_DEVICE_SCRIPT_NAME}. Reason: {e}'
