...
from constants import CREATE_HASH_EXCLUSION_RECORD_SCRIPT_NAME, INTEGRATION_NAME, PRODUCT_NAME
...
def main():
    ...
    siemplify.script_name = CREATE_HASH_EXCLUSION_RECORD_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                results = manager.create_hash_exclusion(hash_value=entity_original_identifier,
                ...
                siemplify.LOGGER.error(
                    "An error occurred on entity: {}: {}".format(entity_original_identifier, err))
                siemplify.LOGGER.exception(err)
        ...
        if successful_entities:
            ...
            output_message = 'Successfully added the following hashes to the exclusion list in {}: \n {}\n'\
                .format(PRODUCT_NAME, '\n'.join(successful_entities))

        if exists_entities:
            output_message += "The following hashes were already a part of exclusion in {}: \n{}\n"\
                .format(PRODUCT_NAME, "\n".join(exists_entities))

        if failed_entities:
            output_message += "Action wasn't able to add the following hashes to the exclusion list in {}: \n{}\n"\
                .format(PRODUCT_NAME, '\n'.join(failed_entities))

        if not successful_entities and not exists_entities:
            output_message = f"No hashes were added to the exclusion list in {PRODUCT_NAME}"
            result_value = False

    except Exception as err:
        output_message = "Error executing action '{}'. Reason: {}".format(CREATE_HASH_EXCLUSION_RECORD_SCRIPT_NAME, err)
