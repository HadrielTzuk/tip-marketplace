...
from constants import INTEGRATION_NAME, ADD_SENDERS_TO_EXCHANGE_SIEMPLIFY_INBOX_RULES, PARAMETERS_DEFAULT_DELIMITER, \
    MAILBOX_DEFAULT_LIMIT, ACTIONS, CONDITIONS, CORRESPONDING_RULES, DOMAIN_RULES
...
def update_rule(logger, manager, rules, email_addresses, mailboxes):
    """
    Find rule and update it with provided email addresses, if rule not found create new rule
...
def main(is_first_run):
    ...
    siemplify.script_name = ADD_SENDERS_TO_EXCHANGE_SIEMPLIFY_INBOX_RULES
    ...
    email_addresses_string = extract_action_parameter(siemplify=siemplify, param_name="Senders",
                                                      print_value=True)
    rule_name = extract_action_parameter(siemplify=siemplify, param_name="Rule to add senders to", is_mandatory=True,
                                         print_value=True)
    add_domains = extract_action_parameter(siemplify=siemplify, param_name="Should add senders' domain to the "
    ...
    try:
        ...
        result_value = False
        output_message = "No email addresses provided in \"Senders\" parameter and there are no email addresses " \
                         "in user entities, Please check action inputs and try again"

        status = EXECUTION_STATE_FAILED
    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {ADD_SENDERS_TO_EXCHANGE_SIEMPLIFY_INBOX_RULES}")
        siemplify.LOGGER.exception(e)
        result_value = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error performing \"{ADD_SENDERS_TO_EXCHANGE_SIEMPLIFY_INBOX_RULES}\" action : {e}"
