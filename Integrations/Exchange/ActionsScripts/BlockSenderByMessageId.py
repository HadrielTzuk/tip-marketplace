...
from constants import INTEGRATION_NAME, BLOCK_SENDER_BY_MESSAGE_ID_SCRIPT_NAME, PARAMETERS_DEFAULT_DELIMITER, \
...
def junk_mail(logger, manager, move_items, message_ids, mailboxes, folder_names, subject_filter, from_filter,
              recipient_filter, mark_all_matching_emails, time_filter):
    """
    Find messages and mark them as junk
    :param logger: {SiemplifyLogger} SiemplifyLogger object.
    :param manager: {ExchangeManager} ExchangeManager object
    :param move_items: {bool} Specifies whether move messages to the junk folder or no
    :param message_ids: {list} The message ids which should be junked
            ...
            if message_ids:
                for message_id in message_ids:
                    try:
                        ...
                        logger.error(f"Failed to mark messages as junk from mailbox {mailbox}.")
                        logger.exception(e)
...
def main(is_first_run):
    ...
    siemplify.script_name = BLOCK_SENDER_BY_MESSAGE_ID_SCRIPT_NAME
    mode = "Main" if is_first_run else "QueryState"
    siemplify.LOGGER.info("----------------- {} - Param Init -----------------".format(mode))

    # Action parameters
    move_items = extract_action_parameter(siemplify=siemplify, param_name="Move item to Junk folder?", is_mandatory=True,
    ...
    mailboxes_string = extract_action_parameter(siemplify=siemplify, param_name="Mailboxes list to perform on",
                                                print_value=True)
    folder_names_string = extract_action_parameter(siemplify=siemplify, param_name="Folder Name", default_value="Inbox",
                                                   print_value=True)
    ...
    mark_all_matching_emails = extract_action_parameter(siemplify=siemplify, param_name="Mark All Matching Emails",
                                                        input_type=bool, print_value=True)
    ...
    try:
        ...
        batch_successful_messages, batch_failed_mailboxes = junk_mail(
        ...
        siemplify.LOGGER.info(f"Junked {len(batch_successful_messages)} messages from "
        ...
        if not not_processed_mailboxes:
            ...
            output_message = f"{len(successful_messages)} email(s) were marked as junk from " \
        ...
        siemplify.LOGGER.error(f"General error performing action {BLOCK_SENDER_BY_MESSAGE_ID_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result_value = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error performing \"{BLOCK_SENDER_BY_MESSAGE_ID_SCRIPT_NAME}\" action : {e}"
...
def prepare_results(successful_messages, failed_mailboxes):
    ...
    output_message += f"\n{len(successful_messages)} mails were successfully marked as junk"
