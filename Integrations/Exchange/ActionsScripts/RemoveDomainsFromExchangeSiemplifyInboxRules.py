...
from constants import INTEGRATION_NAME, REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_INBOX_RULES, \
    PARAMETERS_DEFAULT_DELIMITER, MAILBOX_DEFAULT_LIMIT, ACTIONS, CONDITIONS, DOMAIN_RULES
...
def remove_items_from_rules(logger, manager, rules, domains, mailboxes):
    """
    Find rule and remove provided domains from it
    ...
    for account in accounts:
        ...
        for rule in rules:
            ...
            try:
                ...
                logger.error(f"Failed to update {rule} rule from mailbox {account.primary_smtp_address}.")
                logger.exception(e)
                failed_mailboxes.append(account.primary_smtp_address)
...
def main(is_first_run):
    ...
    siemplify.script_name = REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_INBOX_RULES
    ...
    rule_name = extract_action_parameter(siemplify=siemplify, param_name="Rule to remove Domains from",
                                         is_mandatory=True, print_value=True)
    all_rules = extract_action_parameter(siemplify=siemplify, param_name="Remove Domains from all available Rules",
    ...
    rules = DOMAIN_RULES if all_rules else [rule_name]
    ...
    try:
        ...
        batch_successful_rules, batch_failed_mailboxes = remove_items_from_rules(
        ...
        siemplify.LOGGER.error(f"General error performing action {REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_INBOX_RULES}")
        siemplify.LOGGER.exception(e)
        result_value = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error performing \"{REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_INBOX_RULES}\" action : {e}"
...
def prepare_output_messages(rule_name, domains, failed_mailboxes, processed_mailboxes, not_processed_mailboxes):
    ...
    """
    output_message = f"Removed the following Domains from the corresponding rules: " \
