...
from constants import INTEGRATION_NAME, LIST_EXCHANGE_SIEMPLIFY_INBOX_RULES, PARAMETERS_DEFAULT_DELIMITER, \
...
def list_rules(logger, manager, rules, mailboxes):
    """
    Get rules
    ...
    for account in accounts:
        ...
        try:
            successful_rules.extend(manager.get_rules_by_names(account.primary_smtp_address, rules))
        except Exception as e:
            logger.error(f"Failed to get rules from mailbox {account.primary_smtp_address}.")
            logger.exception(e)
...
def main(is_first_run):
    ...
    siemplify.script_name = LIST_EXCHANGE_SIEMPLIFY_INBOX_RULES
    mode = "Main" if is_first_run else "QueryState"
    siemplify.LOGGER.info("----------------- {} - Param Init -----------------".format(mode))

    # Action parameters
    rule_name = extract_action_parameter(siemplify=siemplify, param_name="Rule Name To List", is_mandatory=True,
                                         print_value=True)
    mailboxes_list = extract_action_parameter(siemplify=siemplify, param_name="Mailboxes list to perform on",
                                              print_value=True)
    ...
    mailboxes_list = [m.strip() for m in mailboxes_list.split(',')] if mailboxes_list else []
    ...
    try:
        ...
        if is_first_run:
            not_processed_mailboxes = mailboxes_list if mailboxes_list else manager.get_searchable_mailboxes_addresses(all_mailboxes)
        ...
        batch_successful_rules, batch_failed_mailboxes = list_rules(
        ...
        siemplify.LOGGER.info(f"Found {len(batch_successful_rules)} rules from "
        ...
        if not not_processed_mailboxes:
            ...
            siemplify.result.add_result_json(json.dumps([rule.to_json() for rule in successful_rules]))
            result_value = True
            ...
            output_message = f"{len(successful_rules)} rule(s) were found from {len(processed_mailboxes)} " \
        ...
        siemplify.LOGGER.error(f"General error performing action {LIST_EXCHANGE_SIEMPLIFY_INBOX_RULES}")
        siemplify.LOGGER.exception(e)
        result_value = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error performing \"{LIST_EXCHANGE_SIEMPLIFY_INBOX_RULES}\" action: {e}"
...
def prepare_output_messages(rule_names, failed_mailboxes, processed_mailboxes, not_processed_mailboxes):
    ...
    """
    output_message = f"Listed the following rules: " \
                     f"\n{PARAMETERS_DEFAULT_DELIMITER.join(rule_names)}" \
                     f"\nfor the specified mailboxes"

    output_message += f"\nSuccessfully listed {len(processed_mailboxes) - len(failed_mailboxes)} mailboxes out of " \
