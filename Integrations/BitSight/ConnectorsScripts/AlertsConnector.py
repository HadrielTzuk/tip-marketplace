...
def main(is_test_run):
    ...
    days_backwards = extract_connector_param(siemplify, param_name="Max Days Backwards", input_type=int,
    ...
    try:
        ...
        if days_backwards < 1:
            siemplify.LOGGER.info(f"Max Days Backwards must be greater than zero. The default value "
        ...
        severity_filter = POSSIBLE_SEVERITIES[POSSIBLE_SEVERITIES.index(lowest_severity):] if \
            lowest_severity in POSSIBLE_SEVERITIES else POSSIBLE_SEVERITIES
        alerts = manager.get_alerts(
            start_timestamp=str(get_last_success_time(
                siemplify=siemplify,
                offset_with_metric={"days": days_backwards},
                time_format=DATETIME_FORMAT).date()),
            severity_filter=convert_list_to_comma_string([severity.upper() for
        ...
        for alert in filtered_alerts:
            try:
                ...
                if not pass_filters(siemplify, whitelist_as_a_blacklist, alert, "trigger"):
                    continue

                if alert.message:
                    alert.findings = manager.get_findings(
                        start_timestamp=alert.alert_date,
                        company_guid=alert.company_id,
                        family_filter=alert.message
                    )
        ...
        if not is_test_run:
            ...
            save_timestamp(siemplify=siemplify, alerts=fetched_alerts, timestamp_key="alert_date_ms")
