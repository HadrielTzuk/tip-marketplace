        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if entity.entity_type == EntityTypes.FILEHASH:
                    af_cookie, af_status = hunt_hash(autofocus_manager, entity.identifier)

                else:
                    # Entity is a FILENAME
                    af_cookie, af_status = autofocus_manager.hunt_file_filename(entity.identifier)
        ...
        for entity_identifier, hunt in hunts.items():
            try:
                if hunt.get(u"completed"):
                    ...
                    if entity.entity_type == EntityTypes.FILEHASH:
                        results, af_status = hunt_hash(autofocus_manager, entity.identifier, hunt[COOKIE])

                    else:
                        # Entity is a FILENAME
                        results, af_status = autofocus_manager.hunt_file_filename(entity.identifier, hunt[COOKIE])
...
def hunt_hash(autofocus_manager, file_hash, af_cookie=None):
    ...
    raise Exception(u"Hash {} is of unknown type (not SHA1, MD5, SHA256).".format(file_hash))
