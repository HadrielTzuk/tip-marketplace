...
from constants import ADD_URL_TO_POLICY_BLOCK_LIST_SCRIPT_NAME, INTEGRATION_NAME, DIRECTION_MAPPER, POLICY_BLOCKED_ENRICHMENT_NAME, ENRICHMENT_PREFIX
    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                manager.add_url_to_policy_block_list(url, category_id, priority, direction, start_port,
                                                     end_port, note, is_regex)
                enriched_entities.append(entity)
                entity.additional_properties.update(add_prefix_to_dict({POLICY_BLOCKED_ENRICHMENT_NAME: "True"}, ENRICHMENT_PREFIX))
                entity.is_enriched = True
                siemplify.LOGGER.info('Successfully blocked the following entity:  \n {}'.format(entity.identifier))
        ...
        if failed_entities:
            output_message += 'Action was not able to block the following entities in the iBoss category with ID {}: \n{}\n'.format(
                category_id, '\n'.join(failed_entities))

        if enriched_entities:
            output_message += 'Successfully blocked the following entities in the iBoss category with ID {} \n{}\n'.format(
                category_id, '\n'.join([entity.identifier for entity in enriched_entities]))
            siemplify.update_entities(enriched_entities)

        else:
            output_message = 'No entities were blocked in the iBoss category with ID {}.'.format(category_id)
        ...
        output_message = "Error executing action '{}'. Reason: {}".format(ADD_URL_TO_POLICY_BLOCK_LIST_SCRIPT_NAME, e)
