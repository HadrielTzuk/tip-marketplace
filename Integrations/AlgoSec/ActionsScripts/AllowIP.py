...
from constants import SLEEP_TIME, INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, ALLOW_IP_SCRIPT_NAME, ALLOW_DEAULT_SUBJECT
...
def main():
    ...
    siemplify.script_name = ALLOW_IP_SCRIPT_NAME
    ...
    subject = extract_action_param(siemplify, param_name="Subject", print_value=True,
                                   default_value=ALLOW_DEAULT_SUBJECT)
    ...
    try:
        ...
        output_message = f"Successfully created a traffic change request to allow traffic to the provided entities " \
                         f"in {INTEGRATION_DISPLAY_NAME}"

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {ALLOW_IP_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action {ALLOW_IP_SCRIPT_NAME}. Reason: {e}"
