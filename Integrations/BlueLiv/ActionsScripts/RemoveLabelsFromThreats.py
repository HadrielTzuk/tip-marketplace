...
from consts import (
    INTEGRATION_NAME,
    REMOVE_LABELS_FROM_THREATS
)

@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = REMOVE_LABELS_FROM_THREATS
        ...
        if len(nonexisting_labels) == len(label_names):
            ...
            if len(nonexisting_threats) == len(threat_ids):
                ...
                for threat in existing_threats:
                    if not removed_labels_from_threats.get(threat):
                        removed_labels_from_threats[threat] = {"success":[], "failed":[]}
                        
                    for label in existing_labels:
                        if label in threats_labels.get(threat):
                            blueliv_manager.add_label_to_threat(label=label, module_id=module_id, module_type=module_type, threat_id=threat)
                            threat_labels_sucess = removed_labels_from_threats[threat].get("success")
                            threat_labels_sucess.append(label)
                            removed_labels_from_threats[threat]["success"] = threat_labels_sucess
                        else:
                            threat_labels_failed = removed_labels_from_threats[threat].get("failed")
                            threat_labels_failed.append(label)
                            removed_labels_from_threats[threat]["failed"] = threat_labels_failed                            

                for threat_id, added_labels in removed_labels_from_threats.items():
                    ...
                    if labels_success:
                        output_message += "\n Successfully removed the following labels from the following threat {} in Blueliv: {}.".format(threat_id,", ".join([label for label in labels_success]))
                    
                    if labels_failed:
                        output_message += "\n The following labels were already not a part of the threat {} in Blueliv: {}.".format(threat_id,", ".join([label for label in labels_failed])) 
                    
                
    except Exception as e:
        output_message = f"Error executing action {REMOVE_LABELS_FROM_THREATS}. Reason: {e}"
