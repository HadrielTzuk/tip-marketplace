...
from consts import INTEGRATION_DISPLAY_NAME, REMOVE_RETENTION_POLICY
...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_DISPLAY_NAME} - {REMOVE_RETENTION_POLICY}"
    ...
    try:
        ...
        manager.remove_retention_policy(log_group_name=log_group_name)
        output_message = f"Successfully removed the retention policy from log group {log_group_name} in " \
                         f"{INTEGRATION_DISPLAY_NAME}"
        status = EXECUTION_STATE_COMPLETED
        result_value = True

    except Exception as e:
        output_message = f"Error executing action '{REMOVE_RETENTION_POLICY}'. Reason: {e}"
