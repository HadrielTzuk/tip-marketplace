...
from consts import INTEGRATION_DISPLAY_NAME, CREATE_LOG_STREAM
...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_DISPLAY_NAME} - {CREATE_LOG_STREAM}"
    ...
    try:
        ...
        manager.create_log_stream(log_group_name=log_group_name, log_stream_name=log_stream_name)
        output_message = f"Successfully created a new log stream {log_stream_name} in log group {log_group_name} in " \
                         f"{INTEGRATION_DISPLAY_NAME}"
        siemplify.result.add_result_json({'log_stream': log_stream_name})
        status = EXECUTION_STATE_COMPLETED
        result_value = True

    except Exception as e:
        output_message = f"Error executing action '{CREATE_LOG_STREAM}'. Reason: {e}"
