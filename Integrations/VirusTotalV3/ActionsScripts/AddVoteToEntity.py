...
from constants import (
    INTEGRATION_NAME,
    ADD_VOTE_TO_ENTITY_SCRIPT_NAME,
    PROVIDER_NAME
)
...
def main():
    ...
    siemplify.script_name = ADD_VOTE_TO_ENTITY_SCRIPT_NAME
    ...
    # action parameters
    vote = extract_action_param(siemplify, param_name="Vote", print_value=False, default_value="Malicious")
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            if entity.entity_type in SUPPORTED_ENTITIES:
                try:
                    ...
                    manager.add_vote_to_entity(entity, vote)
                    status = EXECUTION_STATE_COMPLETED
        ...
        if successful_entities:
            output_message += "Successfully added votes to the following " \
                              "entities in {}: \n {} \n".format(PROVIDER_NAME, ', '.join(successful_entities))
            if failed_entities:
                output_message += "Action wasn't able to add votes to the following " \
                                  "entities in {}: \n {} \n".format(PROVIDER_NAME, ', '.join(failed_entities))
        else:
            result = False
            output_message = f"No votes were added to the provided entities in {PROVIDER_NAME}."

    except Exception as e:
        result = False
        status = EXECUTION_STATE_FAILED
        siemplify.LOGGER.error(f"General error performing action {ADD_VOTE_TO_ENTITY_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        output_message = f"Error executing action \"{ADD_VOTE_TO_ENTITY_SCRIPT_NAME}\". Reason: {e}"
