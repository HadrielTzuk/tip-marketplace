...
from consts import SDK_JOB_ETL_ALERT_DIGESTION_ERRORS as NOTIFICATION_ID


# Providers and strings.
COMPONENT_DISPLAY_NAME = "Alert Digestion Process - ETL"
...
def send_mail(siemplify, mail_message, log_errors):
    try:
        ...
        siemplify.send_mail(MAIL_SUBJECT_PATTERN, mail_message, recipients, "TopEtlError.txt", str(attachment_string))
...
def main():
    ...
    siemplify.script_name = "EtlMonitor"
    siemplify.LOGGER.info("----ETL Monitor started---")
    ...
    try:
        ...
        # Get Notifications.
        faulted_jobs_history = siemplify.get_failed_etljobs(NUMBER_OF_HOURS)

        if not faulted_jobs_history:
            siemplify.LOGGER.info("No error founds for monitoring component {}".format(COMPONENT_DISPLAY_NAME))
        else:
            display_format = "%Y-%m-%d %H:%M:%S UTC"
            starttime_dt = SiemplifyUtils.convert_unixtime_to_datetime(unix_ms_search_from_time).strftime(display_format)
            endtime_dt = SiemplifyUtils.convert_unixtime_to_datetime(unix_ms_now).strftime(display_format)
            msg = "Hello, Siemplify ETL Monitoring service has found errors in component {0} between {1} to {2}.".format(COMPONENT_DISPLAY_NAME, starttime_dt, endtime_dt)
        ...
        siemplify.LOGGER.info("----ETL Monitor finished---")

    except Exception as e:
        siemplify.LOGGER.error("ETL Monitor ERROR. Details: " + e.message)
