...
def main():
    ...
    # action parameters
    request_type = extract_action_param(siemplify, param_name="Request Type", print_value=True)
    query_filters = extract_action_param(siemplify, param_name="Query Filters", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        try:
            results = manager.execute_query(request_type, query_filters, fields_to_return, limit)
        except (CybereasonSuccessWithFailureError, CybereasonClientError):
