...
from constants import INTEGRATION_NAME, PREVENT_FILE_SCRIPT_NAME, MD5
...
def main():
    ...
    siemplify.script_name = PREVENT_FILE_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                siemplify.LOGGER.info(f'Preventing hash {entity_identifier}')
                manager.prevent_file(entity_identifier)
        ...
        if successful_entities:
            output_message = f'Successfully added the following hashes to the blacklist in {INTEGRATION_NAME}: ' \
                              f'{", ".join(successful_entities)}\n'
            if failed_entities:
                output_message += f'Action wasn\'t able to add the following hashes to the blacklist in ' \
                                  f'{INTEGRATION_NAME}: {", ".join(failed_entities)}\n'
        else:
            result_value = False
            output_message = f'No hashes were added to the blacklist in {INTEGRATION_NAME}.'

    except Exception as e:
        output_message = f"Error executing action {PREVENT_FILE_SCRIPT_NAME}. Reason: {e}"
