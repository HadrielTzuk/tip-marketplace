...
from constants import INTEGRATION_NAME, ADD_TAG_TO_AN_ATTRIBUTE, ATTRIBUTE_SEARCH_MAPPER, EXISTING_CATEGORY_TYPES
from exceptions import MISPManagerEventIdNotProvidedError, MISPManagerInvalidCategoryError, \
    MISPManagerObjectUuidProvidedError, MISPManagerEventIdNotFoundError, MISPManagerTagNotFoundError


@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = ADD_TAG_TO_AN_ATTRIBUTE
    ...
    attribute_search = extract_action_param(siemplify, param_name="Attribute Search", print_value=True,
                                            default_value=ATTRIBUTE_SEARCH_MAPPER['provided'])
    ...
    successful_attributes, failed_attributes, json_response = [], [], []

    try:
        manager = MISPManager(api_root, api_token, use_ssl, ca_certificate)

        # Validations
        if attribute_search == ATTRIBUTE_SEARCH_MAPPER['provided'] and not event_id:
        ...
        if attribute_search == ATTRIBUTE_SEARCH_MAPPER['provided'] and event_id:
        ...
        for attribute in attributes:
            ...
            try:
                for tag_name in existing_tag_names:
                    try:
                        json_response.append(manager.add_remove_tag_to_attribute(ADD_ACTION, tag_name, attribute.uuid))
                    except Exception as e:
                        siemplify.LOGGER.error(
                            "Error when adding tag '{}' to an attribute '{}'".format(tag_name, attribute_identifier))
                        siemplify.LOGGER.exception(e)
                successful_attributes.append(attribute_identifier)
            except Exception as e:
                failed_attributes.append(attribute_identifier)
                siemplify.LOGGER.error("Error when adding tags from an attribute '{}'".format(attribute_identifier))
                siemplify.LOGGER.exception(e)
        ...
        if successful_attributes:
            output_message += "Successfully added tags to the following attributes in {}: \n {} \n" \
                .format(INTEGRATION_NAME, ', '.join(successful_attributes))

            if failed_attributes:
                output_message += "Action didn’t add tags to the following attributes in {}: \n {} \n" \
                    .format(INTEGRATION_NAME, ', '.join(failed_attributes))

        else:
            output_message = "No tags were added to the provided attributes in {}\n".format(INTEGRATION_NAME)
            result_value = False
        ...
        output_message = "Error executing action {}. Reason: ".format(ADD_TAG_TO_AN_ATTRIBUTE)
