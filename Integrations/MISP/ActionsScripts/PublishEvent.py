...
def main():
    ...
    try:
        ...
        event = manager.publish_unpublish_event(event_id, publish=True)
        if event.published:
            output_message = 'Successfully published event with {} {} in {}'\
                .format(id_type, event_id, INTEGRATION_NAME)
        else:
            output_message = 'Event with {} {} was not published in {}'.format(id_type, event_id, INTEGRATION_NAME)

        siemplify.result.add_result_json(event.to_json())

    except Exception as e:
        output_message = "Error executing action '{}'. Reason: ".format(PUBLISH_EVENT_SCRIPT_NAME)
