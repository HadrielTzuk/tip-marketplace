    ...
    try:
        ...
        for attribute in manager.get_attributes(attribute_names=attribute_names, attribute_uuids=attribute_uuids,
            ...
            try:
                ...
                siemplify.LOGGER.info('Successfully set IDS flag to the following attribute {}'.format(attribute_uuid_or_value))
            except Exception as e:
                siemplify.LOGGER.error(
                    'Failed to set IDS flag to the following attribute {}'.format(attribute_uuid_or_value))
                siemplify.LOGGER.exception(e)
                failed_attributes.append(attribute_uuid_or_value)

        if successful_attributes:
            output_message += "Successfully set IDS flag for the following attributes in {}:\n {}\n" \
                .format(INTEGRATION_NAME, ', '.join(successful_attributes))

            if failed_attributes:
                output_message += "Action didn’t set IDS flag for the following attributes in {}: \n {} \n" \
                    .format(INTEGRATION_NAME, ', '.join(failed_attributes))
        else:
            output_message = "IDS flag was not set for the provided attributes in {}".format(INTEGRATION_NAME)
            result_value = False

    except Exception as e:
        output_message = "Error executing action {}. Reason: ".format(SET_IDS_FLAG_ON_AN_ATTRIBUTE_SCRIPT_NAME)
