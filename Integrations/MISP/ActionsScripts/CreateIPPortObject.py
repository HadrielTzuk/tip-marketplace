...
from constants import (
    ...
    SOURCE_IP,
    IPPORT_TABLE_NAME,
    CREATE_IPPORT_OBJECT_SCRIPT_NAME
)
...
def main():
    ...
    siemplify.script_name = CREATE_IPPORT_OBJECT_SCRIPT_NAME
    ...
    hostname = extract_action_param(siemplify, param_name="Hostname", print_value=True)
    ...
    try:
        if not use_entities and not (dst_port or src_port or domain or hostname or src_ip or dst_ip):
            raise MISPMissingParamError("One of the: “Dst-port”, “Src-port”, “Domain”, “HOSTNAME”, “IP-Src”, “IP-Dst” "
                                        "should be provided or “Use Entities“ parameter set to true")
        ...
        for params in all_params:
            ...
            try:
                misp_obj = manager.add_ip_port_object(**params)
        ...
        if success_created_objs:
            ...
            siemplify.result.add_data_table(IPPORT_TABLE_NAME.format(event_id),
                                            construct_csv([attribute.to_base_csv() for attribute in all_attributes]))
        if use_entities:
            if success_created_objs:
                output_message = "Successfully created new IP-Port objects for event with {} {} in {} " \
                                 "based on the following entities: \n{}\n" \
                    .format(id_type, event_id, INTEGRATION_NAME, ', '.join(success_created_objs_ids))

                if failed_objects:
                    output_message += "Action wasn’t able to create new IP-Port objects for event with {}" \
                ...
                result_value = False
                output_message = "Action wasn’t able to create new IP-Port objects for event with {} {} in" \
                                 " {} based on the provided entities.".format(id_type, event_id, INTEGRATION_NAME)
        else:
            if success_created_objs:
                output_message = "Successfully created new IP-Port object for event with {} {} in {}." \
                    .format(id_type, event_id, INTEGRATION_NAME)
            elif failed_objects:
                result_value = False
                failed_id, reason = failed_objects[0]
                output_message = "Action wasn’t able to created new IP-Port object for event with {} {} " \
                                 "in {}. Reason: {}".format(id_type, event_id, INTEGRATION_NAME, reason)

    except Exception as e:
        output_message = "Error executing action '{}'. Reason: ". \
            format(CREATE_IPPORT_OBJECT_SCRIPT_NAME)
