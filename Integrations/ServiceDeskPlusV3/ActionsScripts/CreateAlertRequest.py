...
def main():
    ...
    assets = extract_action_param(siemplify, param_name="Assets", is_mandatory=False, input_type=str, default_value="").split(",")
    ...
    try:
        ...
        description = "Siemplify Alert ID: {}, Siemplify Alert Name: {}".format(siemplify.current_alert.external_id, siemplify.current_alert.name)
        
        servicedesk_manager = ServiceDeskPlusManagerV3(api_root=api_root,api_key=api_key, verify_ssl=verify_ssl)
        result = servicedesk_manager.request(action_type=CREATE_REQUEST_TYPE, request_id='',description=description,subject=subject, requester=requester, status=request_status,technician=technician,
                                                          priority=priority,urgency=urgency,category=category, request_template=request_template,
                                                          request_type=request_type,due_by_time=due_by_time, mode=mode,
                                                          level=level, assets=assets, site=site, group=group, impact=impact)
        ...
        output_message = 'Error executing action {}. Reason: {}'.format(CREATE_REQUEST_ALERT_ACTION, e)
