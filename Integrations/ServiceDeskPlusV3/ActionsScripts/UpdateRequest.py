...
def main():
    ...
    subject = extract_action_param(siemplify, param_name="Subject", is_mandatory=False, input_type=str)
    requester = extract_action_param(siemplify, param_name="Requester", is_mandatory=False, input_type=str)
    ...
    try:
        ...
        result = servicedesk_manager.request(action_type=UPDATE_REQUEST_TYPE, request_id=request_id,
                                             description=description, subject=subject, requester=requester,
                                             status=request_status, technician=technician, priority=priority,
                                             urgency=urgency, category=category, request_template=request_template,
                                             request_type=request_type, due_by_time=due_by_time, mode=mode, level=level,
                                             assets=assets, site=site, group=group, impact=impact)
        
        output_message = "Successfully updated ServiceDesk Plus request with ID: {}".format(request_id)
        
        siemplify.result.add_result_json(result.to_json())
        
    except Exception as e:
        output_message = 'Error executing action {}. Reason: {}'.format(UPDATE_REQUEST_ACTION, e)
