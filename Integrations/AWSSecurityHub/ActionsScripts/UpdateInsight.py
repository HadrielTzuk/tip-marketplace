...
def main():
    ...
    insight_name = extract_action_param(siemplify, param_name="Insight Name", is_mandatory=False, print_value=True)

    group_by_attribute = extract_action_param(siemplify, param_name="Group By Attribute", is_mandatory=False,
                                              print_value=True, default_value=None)

    filter_json = extract_action_param(siemplify, param_name="Filter JSON Object", is_mandatory=False,
                                       default_value=None)
    ...
    try:
        ...
        if group_by_attribute and group_by_attribute != 'Select One':  # if group by attribute exists
            ...
            siemplify.LOGGER.info("Group By Attribute parameter is not specified, ignoring parameter.")
        ...
        hub_client.update_insight(insight_arn=insight_arn,
        ...
        result_value = "false"
        siemplify.LOGGER.error(f"Action wasn’t able to update {insight_arn} insight. Reason: {error}")
        siemplify.LOGGER.exception(error)
        output_message += f"Action wasn’t able to update {insight_arn} insight. Reason: {error}"

    except Exception as error:  # action failed
        siemplify.LOGGER.error(f"Error executing action 'Update Insight'. Reason: {error}")
        siemplify.LOGGER.exception(error)
