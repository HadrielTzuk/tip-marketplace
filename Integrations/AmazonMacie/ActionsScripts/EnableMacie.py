...
def main():
    ...
    try:
        ...
        try:
            siemplify.LOGGER.info("Enabling Amazon Macie service")
            manager.enable_macie()
            siemplify.LOGGER.info("Successfully enabled Amazon Macie service")
            output_message = "Successfully enabled Amazon Macie service"
            result_value = "true"
        except Exception as error:
            siemplify.LOGGER.error(f"Failed to enable Amazon Macie service. Error is: {error}")
            siemplify.LOGGER.exception(error)
            output_message = f"Failed to enable Amazon Macie service. Error is: {error}"
