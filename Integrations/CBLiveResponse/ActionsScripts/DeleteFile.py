...
from constants import DELETE_FILE_SCRIPT_NAME, INTEGRATION_NAME, PROVIDER_NAME, FILES_CASE_WALL, VENDOR_NAME
...
from CBLiveResponseActionSteps import session_start, device_custom_output_messages, format_output_message, \
    session_status_check_by_item, get_device_id_by_item
...
def main():
    ...
    file_name = string_to_multi_value(extract_action_param(siemplify, param_name='File Name', print_value=True))
    remote_file_path = string_to_multi_value(extract_action_param(siemplify, param_name='Remote Directory Path',
                                                                  print_value=True))
    ...
    file_names = file_name if file_name else [get_entity_original_identifier(entity)
    ...
    sorted_suitable_entities = sorted(suitable_entities, key=lambda entity: entity.entity_type == EntityTypes.HOSTNAME,
                                      reverse=True)
    ACTION_COMPLETE_OUTPUT_MESSAGES[
        SOME_COMPLETED_MSG] = f"Successfully deleted file {file_name} for the following entities: " + \
                              f"{{{ENTITY_KEY_FOR_FORMAT}}}"
    ACTION_COMPLETE_OUTPUT_MESSAGES[
        SOME_FAILED_MSG] = f"Action failed to delete file {file_name} for the following entities: " + \
                           f"{{{ENTITY_KEY_FOR_FORMAT}}}"
    try:
        if not remote_file_path:
            if not file_names:
                raise Exception("Action failed to start since both Filename Remote Directory Path were not provided.")
            if not is_all_filenames_with_folders(file_names):
        ...
        if remote_file_path:
            if not file_names:
                raise Exception("Action failed to start since both Filename Remote Directory Path were not provided.")
            if not is_all_filenames_without_folders(file_names):
                raise Exception("Action failed to start since some of the elements specified as a placeholder for the "
                                "File Name are full path, and some are not.")

        if remote_file_path and file_names:
            if len(file_names) != len(remote_file_path):
        ...
        action_steps = {
            0: ActionStep(step_id=INITIAL_STEP, step_label='Initial step', method_name='initial_step',
                          variables={'suitable_entities': [get_entity_original_identifier(entity)
                                     ...
                                     'filenames': file_names, 'directory_paths': remote_file_path}),
        ...
        output_message = f"Failed to execute {DELETE_FILE_SCRIPT_NAME} action! Error is {e}"
...
def command_start(step_handler):
    for item in step_handler.get_entities_by_state(state=IN_PROGRESS_ENTITY_STATE):
        ...
        try:
            command = step_handler.manager.start_command_for_delete_file(
...
def collect_json_result(step_handler, item, command, filename, json_result):
    json_result[step_handler.extract_entities_from_item(item)] = json_result.get(
        step_handler.extract_entities_from_item(item), [])
