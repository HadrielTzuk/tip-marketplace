...
from constants import CREATE_MEMDUMP_SCRIPT_NAME, INTEGRATION_NAME, VENDOR_NAME
...
ACTION_COMPLETE_OUTPUT_MESSAGES = {
    ALL_FAILED_MSG: "No memdump files were executed.",
}

FAILED_OUTPUT_MESSAGE = "Action failed to create memdump file {filename} for the following entities: {entities}"
SUCCESSFUL_OUTPUT_MESSAGE = "Successfully created memdump file {filename} for the following entities: {entities}"


@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = CREATE_MEMDUMP_SCRIPT_NAME
    ...
    try:
        ...
        if filenames and remote_dir_path:
        ...
        output_message = f"Failed to execute {CREATE_MEMDUMP_SCRIPT_NAME} action! Error is {e}"
...
def command_start(step_handler, directory_path):
    for item in step_handler.get_entities_by_state(state=IN_PROGRESS_ENTITY_STATE):
        ...
        try:
            ...
            command = step_handler.manager.start_command_to_create_memdump(
...
def command_check(step_handler, json_result):
    for item in step_handler.get_entities_by_state(state=IN_PROGRESS_ENTITY_STATE):
        ...
        try:
            ...
            command = step_handler.manager.get_memdump_command_by_id(
                session_id=session_id,
