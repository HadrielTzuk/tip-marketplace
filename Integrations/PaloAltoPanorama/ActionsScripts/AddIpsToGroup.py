...
SCRIPT_NAME = u"Panorama - AddIpsToGroup"
...
def main():
    ...
    device_group_name = extract_action_param(siemplify, param_name=u"Device Group Name", is_mandatory=True, print_value=True)
    ...
    for entity in siemplify.target_entities:
        if entity.entity_type == EntityTypes.ADDRESS:
            ipsToAdd.add(entity.identifier)
    ...
    if ipsToAdd:
        ...
        for ip in ipsToAdd:
            ...
            try:
                siemplify.LOGGER.info(u"Adding entity: {} to group: {}".format(ip, group))
                ...
                siemplify.LOGGER.info(u"Successfully added entity: {} to group: {}".format(ip, group))
            except Exception as error:
                siemplify.LOGGER.error(u"Unable to add entity: {}. Reason is: {}".format(ip, error))
        ...
        if successful_entities:
            output_message += (u'Successfully added the following IPs to the Palo Alto Panorama address group \"{}\": '
                               u'{}'.format(group, u"\n".join([entity for entity in successful_entities])))

        if failed_entities:
            output_message += u"\n\nAction was not able to add the following IPs to the Palo Alto Panorama address " \
                              u"group \"{}\": {}".format(group, u"\n".join([entity for entity in failed_entities]))

        if not successful_entities:
            output_message = u"No IPs were added to the Palo Alto Panorama address group \"{}\"".format(group)
