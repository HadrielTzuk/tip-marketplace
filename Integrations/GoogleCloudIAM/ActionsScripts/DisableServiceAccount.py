...
from consts import (
    ...
    DISABLE_SERVICE_ACCOUNT_SCRIPT_NAME
)
...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {DISABLE_SERVICE_ACCOUNT_SCRIPT_NAME}"
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                manager.disable_service_account(entity.identifier.strip())
        ...
        if successful_entities:
            output_message += "Successfully disabled the following service accounts:\n   {}".format(
                "\n   ".join([entity.identifier for entity in successful_entities])
            )
            result_value = True
            if failed_entities:
                output_message += "\n\nAction was not able to find a match in {} for the provided entities:\n   {}".format(
                    INTEGRATION_DISPLAY_NAME,
                    "\n   ".join([entity.identifier for entity in failed_entities])
                )
                result_value = False
        else:
            output_message += "No service accounts were disabled."

    except Exception as error:
        output_message = f"Error executing action \"{DISABLE_SERVICE_ACCOUNT_SCRIPT_NAME}\". Reason: {error}"
