...
from consts import (
    ...
    SET_SERVICE_ACCOUNT_IAM_POLICY_SCRIPT_NAME
)
...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {SET_SERVICE_ACCOUNT_IAM_POLICY_SCRIPT_NAME}"
    ...
    try:
        try:
            ...
            raise GoogleCloudIAMPolicyJSONError(error)
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                current_iam_policy = manager.get_service_account_iam_policy(service_account_email=entity.identifier.strip())
                policy_json.update({"etag": current_iam_policy.etag})
                siemplify.LOGGER.info("Setting service account IAM policy for: {}".format(entity.identifier))
                updated_iam_policy = manager.set_service_account_iam_policy(service_account_email=entity.identifier.strip(),
                                                                            policy_json=policy_json)
                json_results[entity.identifier] = updated_iam_policy.to_json()
                successful_entities.append(entity)
                siemplify.LOGGER.info(f"Finished processing entity {entity.identifier}")
            except GoogleCloudIAMPolicyJSONError:
                raise
            except Exception as error:
        ...
        if successful_entities:
            output_message += "Successfully set IAM policy for the following Google Cloud Service Accounts:\n   {}".format(
                "\n   ".join([entity.identifier for entity in successful_entities])
            )
            siemplify.result.add_result_json(convert_dict_to_json_result_dict(json_results))
            result_value = True
            if failed_entities:
                output_message += "\n\nAction was not able to set IAM policy the following Google Cloud Service Accounts:\n   {}".format(
                    "\n   ".join([entity.identifier for entity in failed_entities])
                )
        else:
            output_message += "No Service Account IAM policies were set."

    except GoogleCloudIAMPolicyJSONError as error:
        output_message = f"Provided policy JSON document {policy_json_param} is not valid."
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)

    except Exception as error:
        output_message = f"Error executing action \"{SET_SERVICE_ACCOUNT_IAM_POLICY_SCRIPT_NAME}\". Reason: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
