...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {DELETE_ROLE_SCRIPT_NAME}"
    ...
    try:
        ...
        siemplify.LOGGER.info(f"Searching role")
        role = manager.get_project_role(role_id=role_id)
        siemplify.LOGGER.info("Deleting role")
        deleted_role = manager.delete_projects_role(role_id=role_id, etag=role.etag)
        siemplify.result.add_result_json(deleted_role.to_json())
        output_message = f"{INTEGRATION_DISPLAY_NAME} {role_id} was deleted successfully."
        result_value = True

    except GoogleCloudIAMRoleMissingError as error:
        output_message = f"Provided role id {role_id} does not exists."
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)

    except Exception as error:
        output_message = f"Error executing action \"{DELETE_ROLE_SCRIPT_NAME}\". Reason: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
