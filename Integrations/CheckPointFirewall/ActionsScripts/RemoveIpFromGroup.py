...
from constants import INTEGRATION_NAME, REMOVE_IP_FROM_GROUP_SCRIPT_NAME, PARAMETERS_NEW_LINE_DELIMITER
...
def main():
    ...
    policy_name = extract_configuration_param(siemplify, provider_name=INTEGRATION_NAME, param_name="Policy Name",
                                              is_mandatory=True)
    group_name = extract_action_param(siemplify, param_name='Blacklist Group Name', print_value=True, is_mandatory=True)

    status = EXECUTION_STATE_COMPLETED
    output_message = ""
    result_value = True
    successful_entities, failed_entities = [], []
    relevant_entities = [entity for entity in siemplify.target_entities if entity.entity_type == EntityTypes.ADDRESS]
    ...
    try:
        ...
        for entity in relevant_entities:
            try:
                siemplify.LOGGER.info('\n\nStart process for the following entity: {}'.format(entity))
                manager.unblock_ip_in_policy_group(entity.identifier, group_name)
                successful_entities.append(entity.identifier)
                siemplify.LOGGER.info('Successfully processed the following entity: {}'.format(entity))
            except InvalidGroupException:
                raise
            except Exception as err:
                failed_entities.append(entity.identifier)
                siemplify.LOGGER.error('Action was not able to process the following entity: {}'.format(entity))
                siemplify.LOGGER.exception(err)

            siemplify.LOGGER.info('End process for the following entity: {}'.format(entity))

        if successful_entities:
            output_message += 'Successfully removed the following IPs from the {} Checkpoint FireWall Group: {}\n'\
                .format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(successful_entities))

        if failed_entities:
            output_message += "Action wasn’t able to remove the following IPs from the {} Checkpoint FireWall Group: "\
                             "{}\n".format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(failed_entities))

        if not successful_entities:
            output_message = "No IPs were removed from the {} Checkpoint FireWall Group.".format(group_name)
            result_value = False
        ...
        output_message = 'No IPs were removed from the {} Checkpoint FireWall Group. Reason: {} '\
            .format(group_name, err)
        result_value = False
