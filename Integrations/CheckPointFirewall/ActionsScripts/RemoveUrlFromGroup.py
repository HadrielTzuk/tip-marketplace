...
from constants import INTEGRATION_NAME, REMOVE_URL_FROM_GROUP_SCRIPT_NAME, PARAMETERS_NEW_LINE_DELIMITER
from exceptions import InvalidGroupException

@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = REMOVE_URL_FROM_GROUP_SCRIPT_NAME
    ...
    try:
        ...
        for entity in relevant_entities:
            try:
                ...
                manager.unblock_url_in_group(entity.identifier, group_name)
        ...
        if successful_entities:
            output_message += 'Successfully removed the following URLs from the {} Checkpoint FireWall Group: {}\n'\
                .format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(successful_entities))

        if failed_entities:
            output_message += "Action wasn’t able to remove the following URLs from the {} Checkpoint FireWall Group: "\
                             "{}\n".format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(failed_entities))

        if not successful_entities:
            output_message = "No URLs were removed from the {} Checkpoint FireWall Group.".format(group_name)
            result_value = False
        ...
        output_message = 'No URLs were removed from the {} Checkpoint FireWall Group. Reason: {}'.format(group_name,
                                                                                                         err)
        result_value = False
