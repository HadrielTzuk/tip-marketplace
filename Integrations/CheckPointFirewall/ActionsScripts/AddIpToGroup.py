...
from constants import ADD_IP_TO_GROUP_SCRIPT_NAME, INTEGRATION_NAME, PARAMETERS_NEW_LINE_DELIMITER
...
def main():
    ...
    siemplify.script_name = ADD_IP_TO_GROUP_SCRIPT_NAME
    ...
    try:
        ...
        for entity in relevant_entities:
            try:
                ...
                manager.block_ip_in_policy_group(entity.identifier, group_name)
                ...
                siemplify.LOGGER.error(
                    'Action was not able to process the following entity: {}'.format(entity.identifier))
                siemplify.LOGGER.exception(err)

            siemplify.LOGGER.info('End process for the following entity: {}'.format(entity.identifier))

        if successful_entities:
            output_message += 'Successfully added the following IPs to the {} Checkpoint FireWall Group: \n{}\n' \
                .format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(successful_entities))

        if failed_entities:
            output_message += "Action wasn’t able to add the following IPs to the {} Checkpoint FireWall Group: " \
                              "\n{}\n".format(group_name, PARAMETERS_NEW_LINE_DELIMITER.join(failed_entities))

        if not successful_entities:
            output_message = "No IPs were added to the {} Checkpoint FireWall Group.".format(group_name)
            result_value = False
        ...
        output_message = 'No IPs were added to the {} Checkpoint FireWall Group. Reason: {}'.format(group_name, err)
        result_value = False
