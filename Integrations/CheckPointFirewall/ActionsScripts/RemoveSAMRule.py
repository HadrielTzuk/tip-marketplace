...
from constants import REMOVE_SAM_RULE_SCRIPT_NAME, INTEGRATION_NAME, SLEEP_TIME, REMOVE_SAM_RULE_DEFAULT_MSG
import time
...
def main():
    ...
    target = extract_action_param(siemplify, param_name="Security Gateway", is_mandatory=True,
                                  print_value=True)
    ...
    try:
        ...
        command = checkpoint_manager.construct_remove_sam_rule_command(criteria=criteria, action=action, track_matching_connections=track_matching_connections,
                                                                    close_connections=close_connections)
        siemplify.LOGGER.info("Command: {}".format(command))
        siemplify.LOGGER.info("Initiating run-script command.")
        task_id = checkpoint_manager.run_script(command, [target], script_name=REMOVE_SAM_RULE_DEFAULT_MSG)
        ...
        while not checkpoint_manager.is_task_completed(task_id):
            if unix_now() >= siemplify.execution_deadline_unix_time_ms:
                ...
                output_message = "Timeout waiting for removal of the following SAM rule: {}".format(command)
                break
            ...
            if checkpoint_manager.is_task_succeeded(task_id):
                ...
                output_message = "Successfully removed SAM rule from the Checkpoint Firewall using the command:{}".format(command)
                result_value = True

            elif checkpoint_manager.is_task_succeeded_with_warnings(task_id):
                # Task has completed only partially
                output_message = "SAM rule removal with the following fw sam command succeeded with warnings: {}".format(
                    command)

            elif checkpoint_manager.is_task_partially_succeeded(task_id):
                # Task has completed only partially
                output_message = "SAM rule removal with the following fw sam command partially succeeded: {}".format(
                    command)

            else:
                # Task has failed
                output_message = "Failed to remove SAM rule with the following command: {}".format(command)
        ...
        siemplify.LOGGER.error("Failed to execute Remove SAM Rule action! Error is {}".format(e))
        siemplify.LOGGER.exception(e)
        status = EXECUTION_STATE_FAILED
        result_value = False
        output_message = "Failed to execute Remove SAM Rule action! Error is {}".format(e)
