...
from constants import INTEGRATION_NAME, LAUNCH_SCAN_REPORT_SCRIPT_NAME
...
def main():
    ...
    asset_group_names = extract_action_param(siemplify, param_name="Asset Groups", default_value="", print_value=True)
    asset_group_names = convert_comma_separated_to_list(asset_group_names)
    report_refs = extract_action_param(siemplify, param_name="Scan Reference", print_value=False)
    ...
    try:
      qualys_manager = QualysVMManager(server_address=api_root, username=username, password=password, use_ssl=verify_ssl)
      ...
      output_message = f"Scan report was initialized. Report ID: {report_id}."
        ...
        siemplify.LOGGER.error(f"General error performing action {LAUNCH_SCAN_REPORT_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        report_id = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{LAUNCH_SCAN_REPORT_SCRIPT_NAME}\". Reason: {e}"

    siemplify.LOGGER.info("----------------- Main - Finished -----------------")
    siemplify.LOGGER.info("Status: {}:".format(status))
    siemplify.LOGGER.info("Result Value: {}".format(report_id))
