...
from constants import INTEGRATION_NAME, LAUNCH_PATCH_REPORT_SCRIPT_NAME


@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = LAUNCH_PATCH_REPORT_SCRIPT_NAME
    ...
    try:
        ...
        report_id = qualys_manager.launch_patch_report(
        ...
        output_message = f"Patch report was initialized. Report ID: {report_id}."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {LAUNCH_PATCH_REPORT_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        report_id = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{LAUNCH_PATCH_REPORT_SCRIPT_NAME}\". Reason: {e}"
