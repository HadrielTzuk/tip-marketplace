...
from constants import INTEGRATION_NAME, LIST_REPORTS_SCRIPT_NAME
import json
...
def main():
    ...
    try:
        ...
        if reports:
            ...
            output_message = f"Successfully returned {len(reports)} reports from {INTEGRATION_NAME}."
            siemplify.result.add_result_json(json_results)
        else:
            output_message = f"No reports found in {INTEGRATION_NAME}."
            
    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {LIST_REPORTS_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{LIST_REPORTS_SCRIPT_NAME}\". Reason: {e}"
