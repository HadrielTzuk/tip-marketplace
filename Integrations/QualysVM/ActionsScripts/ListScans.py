...
from constants import INTEGRATION_NAME, LIST_SCANS_SCRIPT_NAME
import json
...
def main():
    ...
    try:
        ...
        if scans:
            ...
            output_message = f"Successfully returned {len(scans)} scans from {INTEGRATION_NAME}."
            siemplify.result.add_result_json(json_results)
        else:
            output_message = f"No scans found in {INTEGRATION_NAME} for past 30 days."
            
    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {LIST_SCANS_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{LIST_SCANS_SCRIPT_NAME}\". Reason: {e}"
