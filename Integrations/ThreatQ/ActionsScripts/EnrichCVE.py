...
SCRIPT_NAME = u"ThreatQ - Enrich CVE"
...
def main():
    ...
    scope_entities = [entity for entity in siemplify.target_entities if entity.entity_type == EntityTypes.CVE]
    ...
    try:
        ...
        for entity in scope_entities:
            try:
                ...
                cve_obj = threatq_manager.get_cve_object(entity.identifier)
                if cve_obj is None:
                    siemplify.LOGGER.info(u"No CVE details ware found for entity: {}".format(entity.identifier))
                    failed_entities.append(entity)
                    continue

                if cve_obj.indicator_score > score_threshold \
                        and (mark_whitelisted_entities_as_suspicious
                             or cve_obj.indicator_status_id != STATUS_MAPPING.get('Whitelisted')):
                    entity.is_suspicious = True

                json_results[entity.identifier] = cve_obj.to_json()
                entity.additional_properties.update(add_prefix_to_dict_keys(cve_obj.to_flat_dict(), THREATQ_PREFIX))
                entities_to_update.append(entity)

                # Case Wall
                flat_comments = cve_obj.comments_table()
                ...
                flat_attributes = cve_obj.attributes_table()
                ...
                flat_sources = cve_obj.sources_table()
        ...
        if entities_to_update:
            ...
            output_message = u"Successfully enriched CVE:\n{0}".format(u", ".join([entity.identifier for entity in
                                                                                   entities_to_update]))
            result_value = u"true"

        else:
            output_message = u"CVEs were not enriched."
