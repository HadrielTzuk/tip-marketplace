...
SCRIPT_NAME = u"ThreatQ - Enrich URL"
    ...
    try:
        ...
        for entity in scope_entities:
            try:
                ...
                url_obj = threatq_manager.get_url_object(entity.identifier)
                if url_obj is None:
                    siemplify.LOGGER.info(u"No URL details ware found for entity: {}".format(entity.identifier))
                    failed_entities.append(entity)
                    continue

                if url_obj.indicator_score > score_threshold \
                        and (mark_whitelisted_entities_as_suspicious
                             or url_obj.indicator_status_id != STATUS_MAPPING.get('Whitelisted')):
                    entity.is_suspicious = True

                json_results[entity.identifier] = url_obj.to_json()
                entity.additional_properties.update(add_prefix_to_dict_keys(url_obj.to_flat_dict(), THREATQ_PREFIX))
                entities_to_update.append(entity)

                # Case Wall
                flat_comments = url_obj.comments_table()
                ...
                flat_attributes = url_obj.attributes_table()
                ...
                flat_sources = url_obj.sources_table()
        ...
        if entities_to_update:
            ...
            output_message = u"Successfully enriched URLs:\n{0}".format(u", ".join([entity.identifier for entity in
                                                                                   entities_to_update]))
            result_value = u"true"

        else:
            output_message = u"URLs were not enriched."
