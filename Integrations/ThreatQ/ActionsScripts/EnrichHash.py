...
SCRIPT_NAME = u"ThreatQ - Enrich Hash"
...
def main():
    ...
    scope_entities = [entity for entity in siemplify.target_entities if entity.entity_type == EntityTypes.FILEHASH]
    ...
    try:
        ...
        for entity in scope_entities:
            try:
                ...
                hash_obj = threatq_manager.get_hash_object(entity.identifier)
                if hash_obj is None:
                    siemplify.LOGGER.info(u"No Hash details ware found for entity: {}".format(entity.identifier))
                    failed_entities.append(entity)
                    continue

                if hash_obj.indicator_score > score_threshold \
                        and (mark_whitelisted_entities_as_suspicious
                             or hash_obj.indicator_status_id != STATUS_MAPPING.get('Whitelisted')):
                    entity.is_suspicious = True

                json_results[entity.identifier] = hash_obj.to_json()
                entity.additional_properties.update(add_prefix_to_dict_keys(hash_obj.to_flat_dict(), THREATQ_PREFIX))
                entities_to_update.append(entity)

                # Case Wall
                flat_comments = hash_obj.comments_table()
                ...
                flat_attributes = hash_obj.attributes_table()
                ...
                flat_sources = hash_obj.sources_table()
        ...
        if entities_to_update:
            ...
            output_message = u"Successfully enriched hashes:\n{0}".format(u", ".join([entity.identifier for entity in
                                                                                      entities_to_update]))
            result_value = u"true"

        else:
            output_message = u"No hashes were enriched."
