...
SCRIPT_NAME = u"ThreatQ - Enrich IP"
    ...
    try:
        ...
        for entity in scope_entities:
            try:
                ...
                ip_obj = threatq_manager.get_ip_object(entity.identifier)
                if ip_obj is None:
                    siemplify.LOGGER.info(u"No IP details ware found for entity: {}".format(entity.identifier))
                    failed_entities.append(entity)
                    continue

                if ip_obj.indicator_score > score_threshold \
                        and (mark_whitelisted_entities_as_suspicious
                             or ip_obj.indicator_status_id != STATUS_MAPPING.get('Whitelisted')):
                    entity.is_suspicious = True

                json_results[entity.identifier] = ip_obj.to_json()
                entity.additional_properties.update(add_prefix_to_dict_keys(ip_obj.to_flat_dict(), THREATQ_PREFIX))
                entities_to_update.append(entity)

                # Case Wall
                flat_comments = ip_obj.comments_table()
                ...
                flat_attributes = ip_obj.attributes_table()
                ...
                flat_sources = ip_obj.sources_table()
        ...
        if entities_to_update:
            ...
            output_message = u"Successfully enriched IP addresses:\n {0}".format(u", ".join([entity.identifier for entity in
                                                                             entities_to_update]))
            result_value = u"true"

        else:
            output_message = u"IP addresses were not enriched."
