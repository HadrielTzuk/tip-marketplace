...
from VirusTotal import VirusTotalManager, FILEHASH_TYPE, ScanStatus, ENTITY_TASK_ID_KEY, ENTITY_REPORT_KEY, \
    ENTITY_STATUS_KEY, VirusTotalInvalidAPIKeyManagerError, VirusTotalLimitManagerError

VT_PREFIX = u"VT"
SCRIPT_NAME = u"VirusTotal - ScanHash"
        ...
        if is_first_run:
            try:
                ...
                if entities_handle:
                    ...
                    output_message += u"No FILE HASH entities were found in current scope."
...
def start_operation(siemplify, manager, rescan_after_days):
    ...
    :return: {entities}
    """
    file_entities = [entity for entity in siemplify.target_entities if entity.entity_type == EntityTypes.FILEHASH]
    entities_handle = {}

    for entity in file_entities:
        try:
            # Search a file hash in virusTotal
            entities_handle.update(manager.define_resource_status(entity.identifier, FILEHASH_TYPE, rescan_after_days))
    ...
    if not file_entities:
        siemplify.LOGGER.info(u"No FILE HASH entities were found in current scope.\n")
...
def query_operation_status(siemplify, manager, threshold, entities_handle):
    ...
    for entity_identifier, entity_handle in entities_handle.items():
        ...
        try:
            if task_id and entity_handle.get(ENTITY_STATUS_KEY) == ScanStatus.QUEUED:
                ...
                if entity_report:
                    siemplify.LOGGER.info(u"Task of {} has completed.")
    ...
    # Action END
            ...
            if entity_handle.get(ENTITY_STATUS_KEY) == ScanStatus.DONE and entity_handle.get(ENTITY_REPORT_KEY):
                ...
                try:
                    ...
                    entity = [e for e in siemplify.target_entities if
                              e.identifier.lower() == entity_identifier.lower()][0]
                    ...
                    if is_risky_entity:
                        siemplify.LOGGER.info(u"Entity {} is risky".format(entity_identifier))
                        is_risky = True
                    entities_to_enrich.append(entity)

                except Exception as err:
                    error_message = u"Error on hash {}: {}.".format(
                        entity_identifier,
                        err.message
                    )
                    siemplify.LOGGER.error(error_message)
        ...
        if report_hashes:
            # Fetch report handle
            output_message += u"Reports were fetched for the following hashes: \n{}\n".format(
                u"\n".join(report_hashes))
...
def add_siemplify_results(siemplify, entity, report, threshold, comments=[]):
    ...
    if int(threshold) <= positives:
        ...
        insight_msg = u'VirusTotal - Hash was marked as malicious by {0} of {1} engines. Threshold set to - {2}'.format(
            report.positives, report.total, threshold)

        siemplify.add_entity_insight(entity, insight_msg, triggered_by=u'VirusTotal')
