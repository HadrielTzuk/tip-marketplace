...
def main():
                ...
                if hash_report.get("occurrences"):
                    for occurrence in hash_report["occurrences"]:
                        ...
                        if host_name not in quarantine_hosts:
                            remediation_items_dict = cynet_manager.quarantine_file_remediation(hash_lower, host_name)
                            quarantine_hosts.append(host_name)
                            remediation_items = remediation_items_dict.get("remediation_items")
    ...
    if remediation_hosts_dict:
        output_message = 'Quarantine file remediation action status for {0}\n'.format(hash_lower)
