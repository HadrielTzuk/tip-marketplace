...
def main():
    ...
    siemplify.script_name = "Cynet - DeleteHashInhosts"
    hash_report = {}
                ...
                if hash_report.get("occurrences"):
                    for occurrence in hash_report["occurrences"]:
                        ...
                        if host_name not in quarantine_hosts:
                            r = cynet_manager.delete_file_remediation(hash_lower, host_name)
    ...
    if remediation_hosts_dict:
        output_message = 'Delete file remediation action status for {0}:\n'.format(hash_lower)
