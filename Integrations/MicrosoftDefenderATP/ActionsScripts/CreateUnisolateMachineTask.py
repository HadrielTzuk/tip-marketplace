...
SCRIPT_NAME = u'{} - Create Unisolate Machine Task'.format(PROVIDER_NAME)
...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                siemplify.LOGGER.info(u'Initiating machine {} unisolation'.format(found_machine.id))
                task = microsoft_defender_atp.unisolate_machine(
                    machine_id=found_machine.id,
                    comment=comment
                )
                tasks.append(task)
                msg = u'Unisolate machine task {} was created successfully on Defender ATP Server for {}'.format(
        ...
        if tasks:
            ...
            siemplify.result.add_data_table(
                title=u'Created Defender ATP unisolate machine tasks:',
