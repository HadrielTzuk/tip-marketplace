...
SCRIPT_NAME = u'{} - Create Stop and Quarantine a File on Specific Machine Task'.format(PROVIDER_NAME)
...
def main():
    ...
    file_hash = extract_action_param(
        siemplify,
        param_name="SHA1 File Hash to Quarantine",
        input_type=unicode,
        is_mandatory=True
    )
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                siemplify.LOGGER.info(u'Initiating stop AV scan and quarantine file on machine {}'.
                                      format(found_machine.id))
                task = microsoft_defender_atp.stop_and_quarantine_machine_file(
                    machine_id=found_machine.id,
                    file_hash=file_hash,
                    comment=comment
                )
                tasks.append(task)
                msg = u'Stop and quarantine file task {} was created successfully on Defender ATP Server for {}'\
                    .format(task.id, entity.identifier)
                output_messages.append(msg)
                ...
                siemplify.LOGGER.exception(e)
                siemplify.LOGGER.error(u'Processing {} entity failed'.format(entity.identifier))
                failed_entities.add(entity)
        ...
        if tasks:
            ...
            siemplify.result.add_data_table(
                title=u'Created Defender ATP "stop and quarantine a file" tasks:',
