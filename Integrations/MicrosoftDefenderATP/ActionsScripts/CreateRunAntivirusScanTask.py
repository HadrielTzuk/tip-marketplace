...
SCRIPT_NAME = u'{} - Create Run Antivirus Scan Task'.format(PROVIDER_NAME)
...
def main():
    ...
    av_scan_type = extract_action_param(
        siemplify,
        param_name="Antivirus Scan Type",
        input_type=unicode,
        is_mandatory=True
    )
    ...
    try:
        MicrosoftDefenderATPManager.validate_av_scan_type(av_scan_type)
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                siemplify.LOGGER.info(u'Initiating run AV scan on machine {}'.format(found_machine.id))
                task = microsoft_defender_atp.run_antivirus_scan(
                    machine_id=found_machine.id,
                    av_scan_type=av_scan_type,
                    comment=comment
                )
                tasks.append(task)
                msg = u'Run AV scan task {} was created successfully on Defender ATP Server for {}'.format(
        ...
        if tasks:
            ...
            siemplify.result.add_data_table(
                title=u'Created Defender ATP "run antivirus scan" tasks:',
