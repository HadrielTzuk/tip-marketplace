...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                if entity.entity_type == EntityTypes.ADDRESS:
                    ...
                    events = cb_defense.get_events_by_hostname(entity.identifier, timeframe)

                if events:
                    siemplify.LOGGER.info(f"{len(events)} events were found for {entity.identifier}.")

                    json_results[entity.identifier] = [event.raw_data for event in events]
                    ...
                    siemplify.LOGGER.info(f"No events were found for {entity.identifier}.")
        ...
        if successful_entities:
            output_message += u"Carbon Black Defense - Events were found for the following entities:\n   {}\n\n".format(
                u"\n   ".join([entity.identifier for entity in successful_entities])
            )

        else:
            output_message += u"No events were found.\n\n"

        if missing_entities:
            output_message += u"No events were found for the following entities:\n   {}\n\n".format(
                u"\n   ".join([entity.identifier for entity in missing_entities])
            )

        if failed_entities:
            output_message += u"Failed to fetch events for the following entities:\n   {}\n\n".format(
