...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, REMOVE_ENTITIES_FROM_ADDRESS_GROUP_SCRIPT_NAME, \
    UPDATE_ACTIONS
...
def main():
    ...
    siemplify.script_name = REMOVE_ENTITIES_FROM_ADDRESS_GROUP_SCRIPT_NAME
    ...
    try:
        ...
        if not address_groups:
            raise Exception(f"address group {address_group_name} was not found in {INTEGRATION_DISPLAY_NAME}. Please "
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                if not existing_entity:
                    ...
                    not_existing_entities.append(entity)
                else:
                    ...
                    if entity_name in [item.get("name") for item in address_group.items]:
                        address_group_items = manager.update_address_group_entities(address_group, address_group_items,
                                                                                    entity_name, UPDATE_ACTIONS.get("remove"))
                        successful_entities.append(entity)
                    else:
                        not_existing_entities.append(entity)
                        siemplify.LOGGER.info(f"{entity.identifier} entity is not a part of the address group"
        ...
        if successful_entities:
            output_message += "Successfully removed the following entities from the address group \"{}\" in {}: \n{}"\
                .format(address_group_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in successful_entities]))

        if not_existing_entities:
            output_message += "\nThe following entities were not a part of the address group {} in {}: \n{}"\
                .format(address_group_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in not_existing_entities]))

        if failed_entities:
            output_message += "\nAction wasn't able to remove the following entities from address group {} in {}: \n{}"\
                .format(address_group_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in failed_entities]))

        if not successful_entities:
            result = False
            output_message = f"None of the provided entities were removed from the address group {address_group_name} " \
                             f"in {INTEGRATION_DISPLAY_NAME}."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {REMOVE_ENTITIES_FROM_ADDRESS_GROUP_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{REMOVE_ENTITIES_FROM_ADDRESS_GROUP_SCRIPT_NAME}\". Reason: {e}"
