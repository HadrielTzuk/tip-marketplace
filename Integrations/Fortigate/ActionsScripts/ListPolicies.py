...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, LIST_POLICIES_SCRIPT_NAME, FILTER_LOGIC_OPERATORS,\
    FILTER_KEY_VALUES
...
def main():
    ...
    try:
        ...
        policies = manager.list_policies(filter_key, filter_logic, filter_value, limit)

        if policies:
            siemplify.result.add_data_table(TABLE_NAME, construct_csv([policy.to_table() for policy in policies]))
            siemplify.result.add_result_json([policy.to_json() for policy in policies])
            output_message = f"Successfully found policies for the provided criteria in {INTEGRATION_DISPLAY_NAME}"
        else:
            result = False
            output_message = f"No policies were found for the provided criteria in {INTEGRATION_DISPLAY_NAME}"
        ...
        siemplify.LOGGER.error(f"General error performing action {LIST_POLICIES_SCRIPT_NAME}")
