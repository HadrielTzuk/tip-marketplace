...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, REMOVE_ENTITIES_FROM_POLICY_SCRIPT_NAME, \
    UPDATE_ACTIONS, ENTITIES_LOCATION
...
def main():
    ...
    siemplify.script_name = REMOVE_ENTITIES_FROM_POLICY_SCRIPT_NAME
    ...
    try:
                ...
                if not existing_entity:
                    ...
                    if entity_name in [item.get("name") for item in items]:
                        policy_items = manager.update_policy_entities(policy, policy_items, entity_name, location,
                                                                      UPDATE_ACTIONS.get("remove"))
                        successful_entities.append(entity)
                    else:
                        not_existing_entities.append(entity)
                        siemplify.LOGGER.info(f"{entity.identifier} entity is not a part of the policy"
                                              f" {policy.name}.")
        ...
        if successful_entities:
            output_message += "Successfully removed the following entities from the policy \"{}\" in {}: \n{}"\
                .format(policy_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in successful_entities]))

        if not_existing_entities:
            output_message += "\nThe following entities were not a part of the policy {} in {}: \n{}"\
                .format(policy_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in not_existing_entities]))

        if failed_entities:
            output_message += "\nAction wasn't able to remove the following entities from policy {} in {}: \n{}"\
                .format(policy_name, INTEGRATION_DISPLAY_NAME,
                        "\n".join([entity.identifier for entity in failed_entities]))

        if not successful_entities:
            result = False
            output_message = f"None of the provided entities were removed from the policy {policy_name} in " \
                             f"{INTEGRATION_DISPLAY_NAME}."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {REMOVE_ENTITIES_FROM_POLICY_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{REMOVE_ENTITIES_FROM_POLICY_SCRIPT_NAME}\". Reason: {e}"
