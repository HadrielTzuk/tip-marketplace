...
def get_recipients(siemplify, ironport_manager, action_start, start_date, end_date, limit, sender, page_size, last_messages_offset=0,
                   recipients=None):
    """
    ...
    try:
        ...
        if status == EXECUTION_STATE_INPROGRESS:
            ...
            output_messages.append('Found {} recipients for the specified email sender {}'.format(len(recipients), sender))

    except IronportAsyncOSMessagesException as e:
        message = 'Failed to get recipients for the specified sender {}. Error is {}'.format(sender, e)
...
def main(is_first_run):
    ...
    # Action parameters
    sender = extract_action_param(siemplify, param_name='Sender', print_value=True, input_type=str, is_mandatory=True)
    ...
    try:
        ...
        if is_first_run:
            ...
            output_message, result_value, status = get_recipients(siemplify, ironport_manager, action_start, start_date, end_date, limit,
                                                                  sender, page_size, last_message_offset, recipients)
