...
import sys
from TIPCommon import dict_to_flat, save_timestamp, get_last_success_time, is_overflowed, read_ids_by_timestamp, \
    write_ids_with_timestamp
from SiemplifyUtils import utc_now, unix_now, convert_string_to_unix_time, convert_unixtime_to_datetime
...
from UtilsManager import load_custom_severity_configuration, map_severity_value, DEFAULT_SEVERITY_VALUE, get_field_value
...
def main(test=False):
    ...
    try:
        ...
        event_class_id_field_name = connector_scope.parameters.get('EventClassId')
        ...
        for alert in alerts:
            try:
                ...
                case = elastic_connector.create_case_info(flat_alert, indexes, query, severity_field_name)
                connector_scope.LOGGER.info(
