...
from constants import INTEGRATION_NAME, REPORT_MESSAGE_ACTION

@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = REPORT_MESSAGE_ACTION
    ...
    comment = extract_action_param(siemplify, param_name="Comment", print_value=True, is_mandatory=False)
    report_as = extract_action_param(siemplify, param_name="Report As", print_value=True, is_mandatory=False, default_value="Spam")
    ...
    try:
        ...
        manager.report_message(message_id=message_id, comment=comment, report_as=report_as)
        
        output_message = f"Successfully reported the following messages in {INTEGRATION_NAME}: \"{message_id}\"."

    except Exception as e:
        output_message = f'Error executing action {REPORT_MESSAGE_ACTION}. Reason: {e}'
