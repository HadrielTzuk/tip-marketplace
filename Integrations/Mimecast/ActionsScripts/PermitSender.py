...
from constants import INTEGRATION_NAME, PERMIT_SENDER_ACTION

@output_handler
def main():
    siemplify = SiemplifyAction()
    siemplify.script_name = PERMIT_SENDER_ACTION
    ...
    try:
        ...
        manager.manage_sender(sender=sender, recipient=recipient, action="permit")
        
        output_message = f"Successfully permitted sender {sender} for recipient {recipient} in {INTEGRATION_NAME}."
        
    except Exception as e:
        output_message = f'Error executing action {PERMIT_SENDER_ACTION}. Reason: {e}'
