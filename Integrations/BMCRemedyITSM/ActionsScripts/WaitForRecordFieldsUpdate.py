...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, WAIT_FOR_RECORD_FIELDS_UPDATE_SCRIPT_NAME
...
def check_record_fields(record, fields_to_check_dict):
    """
    Check record fields
    :param record: {RecordDetails} RecordDetails object
    ...
    for key, value in fields_to_check_dict.items():
        ...
        if record.raw_data.get(key, "") == value:
            updated_fields.append(key)
        else:
            not_updated_fields.append(key)

    if not_found_fields:
        raise Exception(f"the following fields were not found in the structure of record: "
...
def main():
    ...
    siemplify.script_name = WAIT_FOR_RECORD_FIELDS_UPDATE_SCRIPT_NAME
    ...
    fields_to_check = extract_action_param(siemplify, param_name="Fields To Check", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        if is_async_action_timeout(siemplify):
            ...
            if not fail_if_timeout:
                raise BMCRemedyITSMTimeoutException(f"The following fields were not updated for {record_type} record "
                                                    f"with ID {record_id} in {INTEGRATION_DISPLAY_NAME}: "
                                                    f"{','.join(additional_data)}")

            raise Exception(f"The following fields were not updated for {record_type} record with ID {record_id} in "
                            f"{INTEGRATION_DISPLAY_NAME}: {','.join(additional_data)}")

        try:
            fields_to_check_dict = json.loads(fields_to_check)
        except Exception:
        ...
        record = manager.get_record_details(record_type, record_id, fields={})

        if not record:
            raise BMCRemedyITSMNotFoundException(f"record with ID {record_id} was not found")

        updated_fields, not_updated_fields = check_record_fields(record, fields_to_check_dict)

        if len(updated_fields) == len(fields_to_check_dict.keys()):
            siemplify.result.add_result_json(record.to_json())
            result = True
            status = EXECUTION_STATE_COMPLETED
            output_message = f"{record_type} record with ID {record_id} was updated in {INTEGRATION_DISPLAY_NAME}."
        else:
            result = json.dumps(not_updated_fields)
            status = EXECUTION_STATE_INPROGRESS
            output_message = f"Waiting for the following fields to be updated for {record_type} record with ID " \
                             f"{record_id} in {INTEGRATION_DISPLAY_NAME}: {','.join(not_updated_fields)}"
        ...
        siemplify.LOGGER.error(f"General error performing action {WAIT_FOR_RECORD_FIELDS_UPDATE_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action {WAIT_FOR_RECORD_FIELDS_UPDATE_SCRIPT_NAME}. Reason: {e}"
