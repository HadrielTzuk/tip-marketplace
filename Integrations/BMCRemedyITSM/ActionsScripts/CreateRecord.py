...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, CREATE_RECORD_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        record = manager.create_record(record_type=record_type, record_payload=record_payload)
        siemplify.result.add_result_json(record.to_json())

        result = True
        status = EXECUTION_STATE_COMPLETED
        output_message = f"Successfully created {record_type} record in {INTEGRATION_DISPLAY_NAME}."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {CREATE_RECORD_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action {CREATE_RECORD_SCRIPT_NAME}. Reason: {e}"
