...
def load_csv_to_list(csv: str, param_name: str):
    ...
    try:
        ...
        raise Outpost24GeneralException(f"Failed to load comma separated string parameter \"{param_name}\"")
...
def filter_alerts_by_timestamp(logger, alerts, last_success_time, timestamp_key="last_seen"):
    """
    ...
    for alert in alerts:
        ...
        if timestamp >= last_success_time:
            ...
            logger.info(f"The alert {alert.id} is older than {last_success_time}. Skipping...")
    ...
    if lowest_severity:
        ...
        if not filtered_severities:
            siemplify.LOGGER.info(f'Risk is not checked. Invalid value provided for \"Lowest Risk To Fetch\" '
                                  f'parameter. Possible values are: '
        ...
        if filtered_severities and alert.risk_level.lower() not in filtered_severities:
            siemplify.LOGGER.info(f'Finding with risk: {alert.risk_level} did not pass filter. Lowest risk to fetch is '
                                  f'{lowest_severity}.')
            return False
    return True
