...
from constants import CONNECTOR_NAME, DEFAULT_LIMIT, DEFAULT_TIME_FRAME, SEVERITIES, POSSIBLE_TYPES
from UtilsManager import read_ids, write_ids, is_approaching_timeout, get_environment_common, pass_whitelist_filter, \
    ...
    DATETIME_FORMAT, is_overflowed, pass_severity_filter
...
def main(is_test_run):
    ...
    type_filter = extract_connector_param(siemplify, param_name="Type Filter", print_value=True, is_mandatory=True)
    ...
    try:
        ...
        if days_backwards < 1:
            siemplify.LOGGER.info(f"Max Days Backwards must be greater than zero. The default value {DEFAULT_TIME_FRAME} "
                                  f"will be used")
            days_backwards = DEFAULT_TIME_FRAME

        if fetch_limit < 1:
            siemplify.LOGGER.info(f"Max Findings To Fetch must be greater than zero. The default value {DEFAULT_LIMIT} "
                                  f"will be used")
            fetch_limit = DEFAULT_LIMIT

        if lowest_risk_to_fetch and lowest_risk_to_fetch.lower() not in SEVERITIES:
            raise Exception(f"Invalid value given for Lowest Risk To Fetch parameter. Possible values are: "
                            f"{convert_list_to_comma_string([severity.title() for severity in SEVERITIES])}.")

        invalid_types = [type for type in convert_comma_separated_to_list(type_filter) if type.title() not in POSSIBLE_TYPES]

        if invalid_types:
            raise Exception(f"Invalid value provided for the parameter \"Type Filter\". Possible values: "
                            f"{convert_list_to_comma_string(POSSIBLE_TYPES)}.")

        # Read already existing alerts ids
        existing_ids = read_ids(siemplify)
        siemplify.LOGGER.info(f"Successfully loaded {len(existing_ids)} existing findings from ids file")

        manager = Outpost24Manager(api_root=api_root,
                                   username=username,
                                   password=password,
        ...
        for alert in filtered_alerts:
            try:
                ...
                if not pass_filters(siemplify, whitelist_as_a_blacklist, alert, "product_name", lowest_risk_to_fetch):
                    continue
        ...
        if not is_test_run:
            ...
            save_timestamp(siemplify=siemplify, alerts=fetched_alerts, timestamp_key="last_seen")
