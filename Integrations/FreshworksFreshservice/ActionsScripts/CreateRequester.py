...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {CREATE_REQUESTER_SCRIPT_NAME}"
    ...
    primary_email = extract_action_param(siemplify, param_name="Email", print_value=True, is_mandatory=True)
    ...
    try:
        ...
        requester = manager.create_requester(
            **remove_none_dictionary_values(
        ...
        output_message = f"New {INTEGRATION_DISPLAY_NAME} requester created."
        result_value = True

    except (FreshworksFreshserviceValidationError, FreshworksFreshserviceDuplicateValueError) as error:
        output_message = f"Failed to create new requester with the provided parameters! Error is: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)

    except Exception as error:
        output_message = f"Error executing action \"{CREATE_REQUESTER_SCRIPT_NAME}\". Reason: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
