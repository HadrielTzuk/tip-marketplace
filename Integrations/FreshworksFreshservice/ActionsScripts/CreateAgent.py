...
def main():
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {CREATE_AGENT_SCRIPT_NAME}"
    ...
    email = extract_action_param(siemplify, param_name="Email", print_value=True, is_mandatory=True)
    first_name = extract_action_param(siemplify, param_name="First Name", print_value=True, is_mandatory=True)
    ...
    group_membership = extract_action_param(siemplify, param_name="Group Memberships", print_value=True,
                                            is_mandatory=False)
    roles = extract_action_param(siemplify, param_name="Roles", print_value=True, is_mandatory=True)
    ...
    try:
        ...
        agent = manager.create_agent(**remove_none_dictionary_values(
            email=email,
        ...
        output_message = f"New {INTEGRATION_DISPLAY_NAME} agent created."
        result_value = True

    except FreshworksFreshserviceValidationError as error:
        output_message = f"Failed to create new agent with the provided parameters! Error is: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)

    except Exception as error:
        output_message = f"Error executing action \"{CREATE_AGENT_SCRIPT_NAME}\". Reason: {error}"
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(error)
