...
from constants import (
    INTEGRATION_NAME,
    PRODUCT_NAME,
    LIFT_CONTAINED_ENDPOINT_SCRIPT_NAME,
    API_ROOT_DEFAULT,
    DeviceStatusEnum
)
...
def main(is_first_run):
    ...
    siemplify.script_name = LIFT_CONTAINED_ENDPOINT_SCRIPT_NAME
    ...
    try:
        ...
        for entity_type, entity_identifier in result_value['pending'].copy():
            ...
            try:
                ...
                if device.match_status(DeviceStatusEnum.NORMAL.value):
                    result_value['pending'].remove(entity)
                ...
                # skip endpoints with status LIFT_CONTAINMENT_PENDING and process only the rest statuses
                elif not device.match_status(DeviceStatusEnum.LIFT_CONTAINMENT_PENDING.value):
                    siemplify.LOGGER.info(f'Starts lifting containment from {entity_identifier}')
                    manager.lift_containment_from_host_by_device_id(device.device_id)
        ...
        timeout_message = 'The following endpoints initiated containment lift, but were not able to finish it ' \
        ...
        if result_value['pending']:
            if is_timeout_approached:
                ...
                output_message = f'Waiting for containment lift to finish for the following endpoints: ' \
            ...
            for entity_type, identifier in result_value['ready'] + result_value['skipped']:
                # start fetching lifting containment
        ...
        if status != EXECUTION_STATE_INPROGRESS and isinstance(result, dict):
            if result_value['ready']:
                output_message += f'Successfully lifted containment on the following endpoints in {PRODUCT_NAME}: ' \
                                 f'{", ".join([identifier for _, identifier in result_value["ready"]])}\n'
                result_value = True

            if result['skipped']:
                output_message += f'The following endpoints were not contained in {PRODUCT_NAME}: ' \
        ...
        output_message = f"Error executing action '{LIFT_CONTAINED_ENDPOINT_SCRIPT_NAME}'. Reason: {e}"
