...
def main():
    ...
    file_path = extract_action_param(siemplify, param_name=u"File Path", default_value=None, input_type=unicode,
                                   is_mandatory=False)

    file_hash = extract_action_param(siemplify, param_name=u"File Hash", default_value=None, input_type=unicode,
                                   is_mandatory=False)
    ...
    try:
        ...
        if not xdr_manager.is_sha256(file_hash):
            siemplify.LOGGER.error(u"{} is not a valid SHA256 hash. Aborting.".format(file_hash))
            siemplify.end(u"{} is not a valid SHA256 hash. Aborting.".format(file_hash), u"false", EXECUTION_STATE_FAILED)
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                siemplify.LOGGER.info(u"Initiating quarantine of hash {} on endpoint {}".format(file_hash, entity.identifier))
                xdr_manager.quarantine_file_on_endpoint(endpoint.endpoint_id, file_path, file_hash)
        ...
        if successful_entities:
            output_message += u"Successfully initiated quarantine of hash {} on the following of entities:\n   {}".format(
