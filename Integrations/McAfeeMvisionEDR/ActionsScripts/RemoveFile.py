...
SCRIPT_NAME = u"McAfeeMvisionEDR - Remove File"
...
def start_operation(siemplify, manager, full_file_path):
    ...
    output_message = u"File {} wasn't removed from any entities.".format(full_file_path)
    return output_message, u"false", EXECUTION_STATE_COMPLETED


def query_operation_status(siemplify, manager, processed_entities, entities_to_process, failed_entities, failed_entities_with_reason, current_entity, not_found_entities, duplicate_entities, full_file_path, safe_removal):
    """
    Main RemoveFile action
    :param siemplify: SiemplifyAction object
    :param manager: McAfeeMvisionEDRManager object
    ...
    if not current_entity:
        current_entity = process_next_entity(siemplify, entities_to_process, manager, full_file_path, safe_removal)
    ...
    if action_status == COMPLETED_STATUS:
        ...
        output_message = u"Successfully removed {} from the following entity: {}" \
            .format(full_file_path, current_entity.get(u"entity"))
    elif action_status == ERROR_STATUS:
        failed_entities.append(current_entity.get(u"entity"))
        output_message = u"Action wasn't able to remove file {} from the following entity: {}" \
            .format(full_file_path, current_entity.get(u"entity"))
        ...
        output_message = u"Action wasn't able to remove file {} from {}.".format(full_file_path,
    ...
    if processed_entities:
        result_value = u"true"
        output_message += u"Successfully removed {} from the following entities: {}" \
            .format(full_file_path,
                    u"\n".join([processed_entity for processed_entity in processed_entities]))

    if failed_entities:
        output_message += u"\n\nAction wasn't able to remove file {} from the following entities: {}" \
            .format(full_file_path, u"\n".join([failed_entity for failed_entity in failed_entities]))

    if failed_entities_with_reason:
        for item in failed_entities_with_reason:
            output_message += u"\n\nAction wasn't able to remove file {} from {}.".format(full_file_path,
                                                                                          item.get(u"entity"))
...
def main(is_first_run):
    ...
    safe_removal = extract_action_param(siemplify, param_name=u"Safe Removal", is_mandatory=True, input_type=bool,
                                        default_value=True)
    ...
    try:
        ...
        if is_first_run:
            output_message, result_value, status = start_operation(siemplify, mvision_edr_manager, file_full_path)
        else:
            ...
            output_message, result_value, status = query_operation_status(siemplify, mvision_edr_manager,
                                                                          ...
                                                                          current_entity, not_found_entities, duplicate_entities, file_full_path, safe_removal)
