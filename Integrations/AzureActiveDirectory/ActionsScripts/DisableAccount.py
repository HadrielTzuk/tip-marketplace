...
from constants import INTEGRATION_NAME, DISABLE_ACCOUNT_SCRIPT_NAME
...
def main():
    ...
    siemplify.script_name = DISABLE_ACCOUNT_SCRIPT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                manager.disable_user(entity_identifier)
        ...
        if successful_entities:
            output_message = f"Successfully disabled users: \n{', '.join(successful_entities)}\n"

            if failed_entities:
                output_message += f"Failed processing entities:\n{', '.join(failed_entities)}\n"

        else:
            output_message = "No users were disabled."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {DISABLE_ACCOUNT_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
