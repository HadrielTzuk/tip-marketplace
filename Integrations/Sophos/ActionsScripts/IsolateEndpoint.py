...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, ISOLATE_ENDPOINT_SCRIPT_NAME, UNISOLATED, \
...
def start_operation(siemplify, manager, suitable_entities, comment):
        ...
        for entity in suitable_entities:
            try:
                ...
                if initial_isolation_status == ISOLATED:
                    siemplify.LOGGER.info(u"Endpoint for entity {} is already isolated. Skipping.".format(
                        entity.identifier))
                    result_value["already_completed"].append(entity.identifier)
                elif initial_isolation_status == UNISOLATED:
                    manager.isolate_or_unisolate_endpoint(isolate=True, endpoint_id=endpoint.scan_id, comment=comment)
                    final_isolation_status = manager.check_isolation_status(endpoint_id=endpoint.scan_id)
                    if final_isolation_status == ISOLATED:
    ...
    if result_value["pending"]:
        output_message = u"Waiting for isolation to finish on the following entities: " \
...
def finish_operation(siemplify, result_data, timeout_approaching):
    ...
    if successful_entities:
        output_message += u"Successfully isolated the following endpoints in {}: " \
                          u"{}\n".format(INTEGRATION_DISPLAY_NAME,
                                         ', '.join([entity for entity in successful_entities]))

    if already_completed:
        output_message += u"The following endpoints were already isolated in {}: " \
                          u"{}\n".format(INTEGRATION_DISPLAY_NAME,
    ...
    if timeout_approaching and pending_entities:
        ...
        error_message = u"Error executing action {}. Reason: {}".format(ISOLATE_ENDPOINT_SCRIPT_NAME, err)
...
def main(is_first_run):
    ...
    try:
        ...
        output_message = u"Error executing action {}. Reason: {}".format(ISOLATE_ENDPOINT_SCRIPT_NAME, err)
        result_value = False
        status = EXECUTION_STATE_FAILED
