...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, ADD_RECORD_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        record_details = appsheet_manager.add_record(table_name=table_name, query=json_query)
        
        siemplify.result.add_result_json(record_details)
        result = True
        status = EXECUTION_STATE_COMPLETED
        output_message = f"Successfully added new record in table \"{table_name}\" in {INTEGRATION_DISPLAY_NAME}"

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {ADD_RECORD_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{ADD_RECORD_SCRIPT_NAME}\". Reason: {e}"
