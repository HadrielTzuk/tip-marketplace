...
from consts import (
    ...
    SET_NODE_AUTOSCALING_SCRIPT_NAME,
    NOT_CHANGED,
    ENABLED
)
...
def main(is_first_run):
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {SET_NODE_AUTOSCALING_SCRIPT_NAME}"
    ...
    autoscaling_mode = extract_action_param(siemplify, param_name="Autoscaling Mode", default_value=NOT_CHANGED, is_mandatory=False, print_value=True)
    ...
    try:
        min_node_count = extract_action_param(siemplify, param_name="Minimum Node Count", is_mandatory=False, print_value=True, input_type=int)
        max_node_count = extract_action_param(siemplify, param_name="Maximum Node Count", is_mandatory=False, print_value=True, input_type=int)
        ...
        if is_first_run:
            try:
                if not (autoscaling_mode != NOT_CHANGED or isinstance(min_node_count, int) or isinstance(max_node_count, int)):
                    raise MissingParametersException("Provided parameters must specify at least one node autoscaling config.")

                operation = manager.set_cluster_node_pool_autoscaling(zone_name=cluster_location, cluster_name=cluster_name,
                                                                      node_pool_name=node_pool_name,
                                                                      autoscaling_enabled=autoscaling_mode == ENABLED if autoscaling_mode != NOT_CHANGED else None,
                                                                      min_node_count=min_node_count, max_node_count=max_node_count)
        ...
        output_message = f"Error execution action \"{SET_NODE_AUTOSCALING_SCRIPT_NAME}\". Reason: {error}"
