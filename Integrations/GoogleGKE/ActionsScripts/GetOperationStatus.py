...
from consts import (
    ...
    GET_OPERATION_STATUS_SCRIPT_NAME
)
...
def main(is_first_run):
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {GET_OPERATION_STATUS_SCRIPT_NAME}"
    ...
    operation_name = extract_action_param(siemplify, param_name="Operation Name", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        if is_first_run:
            try:
                operation = manager.get_operation(zone_name=cluster_location, operation_name=operation_name)
                if wait_for_operation_to_finish:
                    ...
                    output_message = "Successfully fetched operation details."
                    result_value = True
            except (GoogleGKEInvalidOperationNameError, GoogleGKEInvalidZoneError):
                raise
            ...
            operation = manager.get_operation(cluster_location, operation_name)
            output_message, result_value, status = manager.gke_common.check_operation_status(operation,
                                                                                             is_action_approaching_timeout(
                                                                                                 siemplify.execution_deadline_unix_time_ms))
    ...
    except GoogleGKEInvalidOperationNameError as error:
        ...
        output_message = f"Error execution action \"{GET_OPERATION_STATUS_SCRIPT_NAME}\". Reason: {error}"
