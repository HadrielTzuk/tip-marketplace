...
from consts import (
    ...
    SET_NODE_POOL_MANAGEMENT_SCRIPT_NAME,
    NOT_CHANGED,
    ENABLED,
    DISABLED
)
...
def main(is_first_run):
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {SET_NODE_POOL_MANAGEMENT_SCRIPT_NAME}"
    ...
    auto_upgrade = extract_action_param(siemplify, param_name="Auto Upgrade", default_value=NOT_CHANGED, is_mandatory=False, print_value=True)
    auto_repair = extract_action_param(siemplify, param_name="Auto Repair", default_value=NOT_CHANGED, is_mandatory=False, print_value=True)
    ...
    try:
        ...
        if is_first_run:
            try:
                if auto_upgrade == NOT_CHANGED and auto_repair == NOT_CHANGED:
                    raise MissingParametersException("Provided parameters must specify at least one node pool management config.")

                if not (auto_upgrade != NOT_CHANGED and auto_repair != NOT_CHANGED):
                    node_pool = manager.get_cluster_node_pool(zone_name=cluster_location, cluster_name=cluster_name,
                                                              node_pool_name=node_pool_name)
                    # Setting node pool management config requires providing existing auto upgrade/repair parameters
                    if auto_upgrade == NOT_CHANGED:
                        if isinstance(node_pool.auto_upgrade, bool):
                            auto_upgrade = ENABLED if node_pool.auto_upgrade else DISABLED
                    if auto_repair == NOT_CHANGED:
                        if isinstance(node_pool.auto_repair, bool):
                            auto_repair = ENABLED if node_pool.auto_repair else DISABLED

                operation = manager.set_cluster_node_pool_management(zone_name=cluster_location, cluster_name=cluster_name,
                                                                     node_pool_name=node_pool_name,
                                                                     auto_upgrade=auto_upgrade == ENABLED if auto_upgrade != NOT_CHANGED else None,
                                                                     auto_repair=auto_repair == ENABLED if auto_repair != NOT_CHANGED else None)
        ...
        output_message = f"Error execution action \"{SET_NODE_POOL_MANAGEMENT_SCRIPT_NAME}\". Reason: {error}"
