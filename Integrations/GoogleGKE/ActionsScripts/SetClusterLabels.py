...
from consts import (
    ...
    SET_CLUSTER_LABELS_SCRIPT_NAME
)
...
def main(is_first_run):
    ...
    siemplify.script_name = f"{INTEGRATION_IDENTIFIER} - {SET_CLUSTER_LABELS_SCRIPT_NAME}"
    ...
    cluster_labels = extract_action_param(siemplify, param_name="Cluster Labels", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        if is_first_run:
            try:
                ...
                raise InvalidJSONFormatException(f"Invalid value was provided for the cluster labels: {cluster_labels}")

            cluster = manager.get_cluster(cluster_location, cluster_name)
            if cluster.labels and isinstance(cluster.labels, dict):
                update_labels = cluster.labels
                update_labels.update(cluster_labels)
                cluster_labels = update_labels

            try:
                operation = manager.update_cluster_labels(zone_name=cluster_location, cluster_name=cluster_name,
                                                          label_fingerprint=cluster.label_fingerprint, labels=cluster_labels)
                ...
                siemplify.LOGGER.error(f"Failed to update cluster \"{cluster_name}\" with labels {cluster_labels}")
    ...
    except (InvalidJSONFormatException, GoogleGKEInvalidClusterNameError) as error:
        ...
        output_message = f"Error execution action \"{SET_CLUSTER_LABELS_SCRIPT_NAME}\". Reason: {error}"
