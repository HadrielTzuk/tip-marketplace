...
from constants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, DELETE_IRULE_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        output_message = f"Successfully deleted iRule {name} in {INTEGRATION_DISPLAY_NAME}."

    except InvalidInputException:
        output_message = f"iRule {name} doesn't exist in {INTEGRATION_DISPLAY_NAME}."

    except Exception as e:
        siemplify.LOGGER.error(f"General error performing action {DELETE_IRULE_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        result = False
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action \"{DELETE_IRULE_SCRIPT_NAME}\". Reason: {e}"
