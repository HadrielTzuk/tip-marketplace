...
from constants import (
    ...
    EXECUTE_CUSTOM_SCRIPT_SCRIPT_NAME,
    SUCCESS_STATUS,
    FAILED_STATUS,
    REJECTED_STATUS,
    RUNNING_STATUS,
...
def query_operation_status(
    ...
    for entity_identifier, result_url in results_urls.items():
        ...
        if siemplify.execution_deadline_unix_time_ms - action_start_time < GLOBAL_TIMEOUT_THRESHOLD_IN_MIN * 60_000 or \
                is_approaching_timeout(action_start_time, DEFAULT_TIMEOUT):
            raise TrendVisionOneTimeoutException("action ran into a timeout during execution. ")
        else:
            if task_details.status == SUCCESS_STATUS:
                siemplify.LOGGER.info(
                    f"Successfully executed custom script on {entity_identifier}"
                )
...
def generate_output_message_and_result(result_data):
    ...
    if result_data["completed"]:
        output_message = (
            f"Successfully executed custom script \"{result_data['script_name']}\" "
            f"on the following endpoints in {INTEGRATION_DISPLAY_NAME}: "
            f"{', '.join([entity for entity in result_data['completed']])}\n"
        )
        if result_data["failed"]:
            output_message += (
                f"Action wasn’t able to execute custom script \"{result_data['script_name']}\" "
                f"on the following endpoints in {INTEGRATION_DISPLAY_NAME}: "
                f"{', '.join([entity for entity in result_data['failed']])}\n"
            )
            result_value = False
    else:
        output_message = "Scripts were not executed on the provided endpoints."
        result_value = False
...
def main(is_first_run):
    ...
    siemplify.script_name = EXECUTE_CUSTOM_SCRIPT_SCRIPT_NAME
    ...
    script_name = extract_action_param(
        siemplify,
        param_name="Script Name",
        print_value=True,
        is_mandatory=True
    )
    script_parameters = extract_action_param(
        siemplify,
        param_name="Script Parameters",
        print_value=True
    )
    ...
    try:
        if not suitable_entities:
            ...
            script = manager.get_script_by_name(
                script_name=script_name
            )

            if not script:
                raise Exception(f"script with name \"{script_name}\" wasn’t found")
        ...
        output_message = 'Error executing action {}. Reason: {}'.format(EXECUTE_CUSTOM_SCRIPT_SCRIPT_NAME, e)
        status = EXECUTION_STATE_FAILED
        result_value = False
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
