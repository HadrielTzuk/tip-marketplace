...
def main():
    ...
    siemplify.script_name = LIST_URI_LISTS_SCRIPT_NAME
    ...
    # Parameters
    groups_limit = extract_action_param(siemplify, param_name=u'Max URI Lists To Return', default_value=MAX_LIMIT,
    ...
    try:
        ...
        results = sonic_wall_manager.get_uri_lists()
        if results:
            ...
            for uri_list_object in results:
                row_data = {u'Name': uri_list_object.name, u'URI Count': len(uri_list_object.uri_list)}
            ...
            siemplify.result.add_data_table(u'Available URI Lists', csv_output)
            siemplify.result.add_result_json(json_results)
            output_message = u'Successfully listed SonicWall URI Lists!'
            result_value = u'true'
        else:
            output_message = u'No SonicWall URI Lists were found!'
        ...
        output_message = u"Error executing action \"List URI Lists\". Reason: {}".format(e)
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
        status = EXECUTION_STATE_FAILED
