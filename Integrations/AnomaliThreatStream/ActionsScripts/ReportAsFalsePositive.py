...
from constants import INTEGRATION_NAME, REPORT_AS_FALSE_POSITIVE_SCRIPT_NAME, EMAIL_TYPE
...
def main():
    ...
    siemplify.script_name = REPORT_AS_FALSE_POSITIVE_SCRIPT_NAME
    ...
    reason = extract_action_param(siemplify, param_name="Reason", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        if indicators:
            for entity in suitable_entities:
                ...
                try:
                    ...
                    manager.report_as_false_positive(indicator_id=entity_indicator.id, reason=reason, comment=comment)
        ...
        if successful_entities:
            output_message = f"Successfully reported the following entities as false positive in {INTEGRATION_NAME}: " \
                             f"{', '.join(successful_entities)}\n"

            if failed_entities:
                output_message += "Action was not able to report the following entities as false positive in " \
                                  f"{INTEGRATION_NAME}: {', '.join(failed_entities)}"

        else:
            output_message = "No entities were reported as false positive."
            result_value = False

    except Exception as e:
        output_message = f"Error executing action '{REPORT_AS_FALSE_POSITIVE_SCRIPT_NAME}'. Reason: {e}"
