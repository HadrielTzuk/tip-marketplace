...
def main():
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            siemplify.LOGGER.info(f"Started processing entity: {entity_original_identifier}")
            try:
                if manager.group:
                ...
                siemplify.result.add_entity_table(
                    ...
                    construct_csv([
                        agent_dat.to_csv(entity_identifier=entity_original_identifier),
                        server_dat.to_csv()
                    ]))
        ...
        if success_entities:
            ...
            output_message = 'Successfully retrieved server and agent DAT information from the following endpoints ' \
                             f'in {PRODUCT_NAME}: {", ".join(success_entities)}\n'
            if failed_entities:
                output_message += 'Action wasn\'t able to retrieve server and agent DAT information from the ' \
