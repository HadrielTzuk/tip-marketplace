...
from constants import INTEGRATION_NAME, PRODUCT_NAME, REMOVE_TAG_SCRIPT_NAME
...
def main():
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                if manager.group:
                    ...
                    affected = manager.clear_tag_from_endpoint_by_host_id(tag_name, host_id=system.parent_id)
                else:
                    affected = manager.clear_tag_from_endpoint_by_host_name(tag_name,
                                                                            host_name=entity_original_identifier)
                ...
                siemplify.LOGGER.error(f'Failed to remove tag for entity {entity_original_identifier}')
        ...
        if success_entities:
            output_message = f'Successfully removed tag "{tag_name}" from the following endpoints in ' \
                             f'{PRODUCT_NAME}: {", ".join(success_entities)}\n'
        if failed_entities:
            output_message += f'Action wasn\'t able to remove tag "{tag_name}" from the following endpoints in ' \
                             f'{PRODUCT_NAME}: {", ".join(failed_entities)}\n'
        if ignored_entities:
            output_message += f'Tag "{tag_name}" wasn\'t a part of the following endpoints in ' \
                              f'{PRODUCT_NAME}: {", ".join(ignored_entities)}\n'
        elif not success_entities:
            result_value = False
            output_message = f'Tag "{tag_name}" wasn\'t removed from the provided endpoints.'
    except Exception as e:
        output_message = f"Error executing action '{REMOVE_TAG_SCRIPT_NAME}'. Reason: {e}"
