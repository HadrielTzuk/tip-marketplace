...
from constants import INTEGRATION_NAME, GET_HOST_NETWORK_IPS_STATUS_SCRIPT_NAME, \
    HOST_NETWORK_IPS_STATUS_TABLE_NAME, PRODUCT_NAME
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                ...
                if hip_property:
                    json_result[entity_identifier] = hip_property.to_json(status_key=JSON_RESULT_STATUS_KEY)
                    csv_result.append(hip_property.to_csv(entity_identifier=entity_identifier,
        ...
        if successful_entities:
            output_message += "Successfully retrieved Network IPS information from the following endpoints in " \
                              f"{PRODUCT_NAME}: {', '.join(successful_entities)} \n"
            siemplify.result.add_data_table(HOST_NETWORK_IPS_STATUS_TABLE_NAME, construct_csv(csv_result))
            siemplify.result.add_result_json(convert_dict_to_json_result_dict(json_result))

            if failed_entities:
                output_message += "Action wasn't able to retrieve Network IPS information from the following " \
                                  f"endpoints in {PRODUCT_NAME}: {', '.join(failed_entities)}"
        else:
            output_message = "No information about Network IPS was found on the provided endpoints."
            result_value = False
