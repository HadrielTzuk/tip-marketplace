...
from datamodels import Indicator, Association, Vulnerability, JobDetails, JobStatus, Tag, AnalysisLink, \
    ActorDetails, CampaignDetails, IntelDetails, IndicatorsGroup, SignatureDetails, AttackPatternDetails, \
    MalwareDetails, IncidentDetails, IdentityDetails, ThreatBulletinsDetails, CourseOfActionDetails, \
    InfrastructureDetails, \
    IntrusionSetDetails, ToolDetails, TTPDetails
from utils import slice_list_to_max_sublists

import pandas as pd

class ThreatFuseParser(object):
    """
    ...
    def build_indicator_obj(objects_data, web_root):
        return Indicator(
            ...
            tags=[ThreatFuseParser.build_tag_obj(tag) for tag in (objects_data.pop('tags', []) or [])],
    ...
    def build_association_obj(objects_data, web_root):
        ...
        aliases_objs = [ThreatFuseParser.build_alias_obj(alias) for alias
                        in objects_data.get("aliases", []) if isinstance(alias, dict)]
    ...
    def build_vulnerability_obj(objects_data, web_root):
        ...
        objects_data = ThreatFuseParser.shorten_body_and_description(objects_data)
    ...
    def build_actor_details_obj(actor_details, web_root):
        ...
        actor_details = ThreatFuseParser.shorten_body_and_description(actor_details)
        ...
        return ActorDetails(
            ...
            aliases_obj=[ThreatFuseParser.build_alias_obj(alias) for alias
                         in actor_details.get("aliases", [])],
            web_root=web_root,
            status_display_name=status_display_name,
            **actor_details
        )
    ...
    def build_campaign_details_obj(campaign_details, web_root):
        ...
        campaign_details = ThreatFuseParser.shorten_body_and_description(campaign_details)

        return CampaignDetails(
            raw_data=campaign_details,
            aliases_obj=[ThreatFuseParser.build_alias_obj(alias) for alias
                         in campaign_details.get("aliases", [])],
            status_display_name=status_display_name,
            web_root=web_root,
            **campaign_details
        )
    ...
    def build_signature_details_obj(signature_details, web_root):
        ...
        signature_details = ThreatFuseParser.shorten_body_and_description(signature_details)

        return SignatureDetails(
            raw_data=signature_details,
            aliases_obj=[ThreatFuseParser.build_alias_obj(alias) for alias
                         in signature_details.get("aliases", [])],
    ...
    @staticmethod
    def match_entity_to_indicators(indicators: List[Indicator], entities: List[DomainEntityInfo]) -> List[
        IndicatorsGroup]:
        """
    ...
    @staticmethod
    def map_observables_to_feed_group(observables: List[Indicator], max_observables_per_group: int,
                                      source_feed_grouping: bool):
        """
        Group observables. Each group will represent an alert of max size <max_observable_per_group>,
        if source_feed_grouping is True alerts will be grouped by the same feed_id.
        :param observables: {[Indicator]} List of observables to group
        :param max_observables_per_group:  {int} max observables per group.
        :param source_feed_grouping: {bool} if True, observables will be grouped by feed_id attribute
        :return: {[[datamodels.Indicator]]} List of sublists of indicators. Each inner list represents observables that will be processed
            in the same alert as events. Max observables per group is defined by the parameter <max_observables_per_group>
        """
        if source_feed_grouping:  # sort observables by feed id
            observable_groups = []
            # split observables into groups with the same feed_d
            for feed_id, group in itertools.groupby(observables, lambda k: k.feed_id):
                # split each group to subgroups with max size <max_observables_per_group>
                for sub_group in slice_list_to_max_sublists(list(group), max_observables_per_group):
                    observable_groups.append(sub_group)

            return observable_groups

        return slice_list_to_max_sublists(observables, max_observables_per_group)
    ...
    def build_attackpattern_details_obj(attack_pattern_details, web_root):
        ...
        attack_pattern_details = ThreatFuseParser.shorten_body_and_description(attack_pattern_details)
    ...
    def build_malware_details_obj(malware_details, web_root):
        ...
        malware_details = ThreatFuseParser.shorten_body_and_description(malware_details)
    ...
    def build_incident_details_obj(incident_details, web_root):
        ...
        incident_details = ThreatFuseParser.shorten_body_and_description(incident_details)
    ...
    def build_threat_bulletins_details_obj(threat_bulletins_details, web_root):
        ...
        threat_bulletins_details = ThreatFuseParser.shorten_body_and_description(threat_bulletins_details)
    ...
    def build_course_of_action_details_obj(course_of_action_details, web_root):
        ...
        course_of_action_details = ThreatFuseParser.shorten_body_and_description(course_of_action_details)
    ...
    def build_identity_details_obj(identity_details, web_root):
        ...
        identity_details = ThreatFuseParser.shorten_body_and_description(identity_details)
    ...
    def build_infrastructure_details_obj(infrastructure_details, web_root):
        ...
        infrastructure_details = ThreatFuseParser.shorten_body_and_description(infrastructure_details)
    ...
    def build_intrusionset_details_obj(intrusionset_details, web_root):
        ...
        intrusionset_details = ThreatFuseParser.shorten_body_and_description(intrusionset_details)
    ...
    def build_tool_details_obj(tool_details, web_root):
        ...
        tool_details = ThreatFuseParser.shorten_body_and_description(tool_details)
    ...
    def build_ttp_details_obj(ttp_details, web_root):
        ...
        ttp_details = ThreatFuseParser.shorten_body_and_description(ttp_details)
    ...
    def shorten_body_and_description(raw_data):
        ...
        if isinstance(raw_data.get("description", ""), str):
            splitted = ' '.join(
                raw_data.get("description", "").split(' ')[:consts.MAX_DESCRIPTION_LENGTH])
            if splitted:
        ...
        if isinstance(raw_data.get("body", ""), str):
            splitted = ' '.join(
                raw_data.get("body", "").split(' ')[:consts.MAX_BODY_LENGTH])
            if splitted:
    ...
    def build_attribute_statistics_obj(raw_data, meta_total_count, limit=None):
        ...
        country_stats = objects.groupby(["country"], as_index=False).size().rename(columns={'country':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        type_stats = objects.groupby(["type"], as_index=False).size().rename(columns={'type':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        threat_type_stats = objects.groupby(["threat_type"], as_index=False).size().rename(columns={'threat_type':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        source_stats = objects.groupby(["source"], as_index=False).size().rename(columns={'source':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        status_stats = objects.groupby(["status"], as_index=False).size().rename(columns={'status':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        severity_stats = pd.DataFrame([v for k,v in objects["meta"].items()]).groupby(["severity"], as_index=False).size().rename(columns={'severity':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
        org_stats = objects.groupby(["org"], as_index=False).size().rename(columns={'org':'name','size':'count'}).sort_values(['count'], ascending=False).replace('',"N/A").to_dict(orient="records")
