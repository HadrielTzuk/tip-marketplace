...
SCRIPT_NAME = "Get Related Domains"
...
def main():
    ...
    max_domains_to_return = extract_action_param(siemplify, param_name="Max Domains To Return", is_mandatory=False,
                                                 print_value=True, default_value=consts.MAX_DOMAINS_TO_RETURN_DEFAULT,
                                                 input_type=int)

    siemplify.LOGGER.info("----------------- Main - Started -----------------")

    if max_domains_to_return < 0:
        siemplify.LOGGER.info(
            f"\"Max Domains To Return\" must be non-negative. Using default of {consts.MAX_DOMAINS_TO_RETURN_DEFAULT}.")
        max_domains_to_return = consts.MAX_DOMAINS_TO_RETURN_DEFAULT
    ...
    try:
        ...
        for association_type in search_association_types:
            ...
            if get_max_dict_value_size(json_results) >= max_domains_to_return:
                siemplify.LOGGER.info(f"Json results size reached domains max size of {max_domains_to_return}")
                break
            ...
            try:
                ...
                for association in related_associations:
                    if get_max_dict_value_size(json_results) >= max_domains_to_return:
                        siemplify.LOGGER.info(
                            f"Json results size reached domains max size to return of {max_domains_to_return}. Skipping association {association.id}")
                        break

                    query_limit = max_domains_to_return - get_max_dict_value_size(json_results)
                    association_indicators = manager.get_association_type_indicators(
                        association_type=association_type,
                        association_id=association.id,
                        indicator_type=consts.DOMAIN_INDICATOR_TYPE,
                    ...
                    for indicator in association_indicators:
                        json_results["domains"].add(indicator.value)

                    result_value = "true"
                    siemplify.LOGGER.info(
                        f"Successfully got domains for association {association_type} with name {association.name} and id {association.id}")
        ...
        for entity in actors_and_vulnerabilities:
            ...
            try:
                ...
                result_value = "true"
                siemplify.LOGGER.info(
                    f"Successfully got domains for association {model_type} with name {association.name} and id {association.id}")
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(f"Successfully retrieved related domains from {INTEGRATION_NAME}")
            output_message = f"Successfully retrieved related domains from {INTEGRATION_NAME}"
        else:
            siemplify.LOGGER.info("No related domains were found")
            output_message = "No related domains were found"
