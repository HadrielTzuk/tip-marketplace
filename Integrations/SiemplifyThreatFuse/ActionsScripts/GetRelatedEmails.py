...
SCRIPT_NAME = "Get Related Email Addresses"
...
def main():
    ...
    max_email_addresses_to_return = extract_action_param(siemplify, param_name="Max Email Addresses To Return",
                                                         is_mandatory=False,
                                                         print_value=True,
                                                         default_value=consts.MAX_EMAIL_ADDRESSES_TO_RETURN_DEFAULT,
                                                         input_type=int)

    siemplify.LOGGER.info("----------------- Main - Started -----------------")

    if max_email_addresses_to_return < 0:
        siemplify.LOGGER.info(
            f"\"Max Email Addresses To Return\" must be non-negative. Using default of {consts.MAX_EMAIL_ADDRESSES_TO_RETURN_DEFAULT}.")
        max_email_addresses_to_return = consts.MAX_EMAIL_ADDRESSES_TO_RETURN_DEFAULT
    ...
    try:
        ...
        siemplify.LOGGER.info(
            f"Supported entities are: {supported_entities + [entity.identifier for entity in actors_and_vulnerabilities]}"
        )
        ...
        for association_type in search_association_types:
            ...
            if get_max_dict_value_size(json_results) >= max_email_addresses_to_return:
                siemplify.LOGGER.info(f"Json results size reached hashes max size of {max_email_addresses_to_return}")
                break
            ...
            try:
                ...
                related_associations = manager.get_related_indicator_associations(
                    ...
                    limit=max_email_addresses_to_return,
                    asc=False
                )
                ...
                for association in related_associations:
                    if get_max_dict_value_size(json_results) >= max_email_addresses_to_return:
                        siemplify.LOGGER.info(
                            f"Json results size reached emails max size of {max_email_addresses_to_return}. Skipping association {association.id}")
                        break

                    query_limit = max_email_addresses_to_return - get_max_dict_value_size(json_results)
                    association_indicators = manager.get_association_type_indicators(
                        association_type=association_type,
                        association_id=association.id,
                        indicator_type=consts.EMAIL_INDICATOR_TYPE,
                    ...
                    for indicator in association_indicators:
                        json_results["emails"].add(indicator.value)

                    result_value = "true"
                    siemplify.LOGGER.info(
                        f"Successfully got email addresses for association {association_type} with name {association.name} and id {association.id}")
        ...
        for entity in actors_and_vulnerabilities:
            ...
            try:
                ...
                if get_max_dict_value_size(json_results) >= max_email_addresses_to_return:
                    siemplify.LOGGER.info(
                        f"Json results size reached hashes max size of {max_email_addresses_to_return}")
                    break
                ...
                result_value = "true"
                siemplify.LOGGER.info(
                    f"Successfully got email addresses for association {model_type} with name {association.name} and id {association.id}")
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(f"Successfully retrieved related email addresses from {INTEGRATION_NAME}")
            output_message = f"Successfully retrieved related email addresses from {INTEGRATION_NAME}"
        else:
            siemplify.LOGGER.info("No related email addresses were found")
            output_message = "No related email addresses were found"
