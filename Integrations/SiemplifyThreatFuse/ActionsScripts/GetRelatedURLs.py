...
def main():
    ...
    max_urls_to_return = extract_action_param(siemplify, param_name="Max URLs To Return", is_mandatory=False,
                                              print_value=True, default_value=consts.MAX_URLS_TO_RETURN_DEFAULT,
                                              input_type=int)
    ...
    if max_urls_to_return < 0:
        siemplify.LOGGER.info(
            f"\"Max URLs To Return\" must be non-negative. Using default of {consts.MAX_URLS_TO_RETURN_DEFAULT}.")
        max_urls_to_return = consts.MAX_URLS_TO_RETURN_DEFAULT
    ...
    try:
        ...
        for association_type in search_association_types:
            ...
            if get_max_dict_value_size(json_results) >= max_urls_to_return:
                siemplify.LOGGER.info(f"Json results size reached urls max size of {max_urls_to_return}")
                break
            ...
            try:
                ...
                for association in related_associations:
                    if get_max_dict_value_size(json_results) >= max_urls_to_return:
                        siemplify.LOGGER.info(
                            f"Json results size exceeds urls max size of {max_urls_to_return}. Skipping association {association.id}")
                        break

                    query_limit = max_urls_to_return - get_max_dict_value_size(json_results)
                    association_indicators = manager.get_association_type_indicators(
                        association_type=association_type,
                        association_id=association.id,
                        indicator_type=consts.URL_INDICATOR_TYPE,
                    ...
                    for indicator in association_indicators:
                        json_results["urls"].add(indicator.value)

                    result_value = "true"
                    siemplify.LOGGER.info(
                        f"Successfully got urls for association {association_type} with name {association.name} and id {association.id}")
        ...
        for entity in actors_and_vulnerabilities:
            ...
            try:
                ...
                result_value = "true"
                siemplify.LOGGER.info(
                    f"Successfully got urls for association {model_type} with name {association.name} and id {association.id}")
        ...
        if get_max_dict_value_size(json_results) > 0:
            siemplify.LOGGER.info(f"Successfully retrieved related urls from {INTEGRATION_NAME}")
            output_message = f"Successfully retrieved related urls from {INTEGRATION_NAME}"
        else:
            siemplify.LOGGER.info("No related urls were found")
            output_message = "No related urls were found"
