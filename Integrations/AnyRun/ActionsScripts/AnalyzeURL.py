...
def start_operation(siemplify, anyrun_manager, wait_for_report, element_for_analysis, os_version ,operation_system_bitness, os_env_type,
    ...
    :param element_for_analysis: {str} URL that should be analyzed
    ...
    try:
        task = anyrun_manager.analyze(element_type=URL_ELEMENT, element_for_analysis=element_for_analysis, os_version=os_version ,operation_system_bitness=operation_system_bitness, os_env_type=os_env_type,
        ...
        if task.task_id:
            ...
            output_message = "Created analysis tasks for the following URL: {}".format(element_for_analysis)
        else:
            result_value = False
            output_message = "No Any.Run analysis tasks were created for the following URL: {}".format(element_for_analysis)
            status = EXECUTION_STATE_COMPLETED

    except AnyRunError as e:
        output_message = "Action reached timeout waiting for report for URL: {0}".format(element_for_analysis)
        ...
        output_message = 'Error executing action {}. Reason: {}'.format(ANALYZE_URL_ACTION, e)
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
        status = EXECUTION_STATE_FAILED
        result_value = False
...
def query_operation_status(siemplify, anyrun_manager, element_for_analysis):
    ...
    if analysis_status == STATUS_IN_PROGRESS:
        ...
        output_message = "Report of the URL {} with ID: {} is not ready yet. Will check again later....".format(element_for_analysis, task_id) 
    
    else:
        status = EXECUTION_STATE_COMPLETED    
        result_value = True
        output_message = "Report of the URL analysis successfully prepared."
...
def main(is_first_run):
    ...
    siemplify.script_name = ANALYZE_URL_ACTION
    ...
    url_for_analysis = extract_action_param(siemplify, param_name="URL For Analysis", is_mandatory=True, print_value=True)
