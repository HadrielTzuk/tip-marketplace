...
SCRIPT_NAME = u"DigitalShadows - EnrichUrl"
...
def main():
    ...
    try:
        ...
        if target_entities:
            for entity in target_entities:
                if unix_now() >= siemplify.execution_deadline_unix_time_ms:
                    ...
                    break
                try:
                    url_obj = manager.enrich_url(entity.identifier)
                    successful_entities.append(entity)
                    json_results[entity.identifier] = url_obj.to_json()
                    ## enrich the entity
                    siemplify.result.add_entity_table(entity.identifier, url_obj.to_csv())
                    siemplify.result.add_entity_link(entity.identifier, url_obj.link)
                    entity.additional_properties.update(url_obj.to_enrichment_data())
            ...
            if failed_entities:
                output_message += u"Failed to enrich URLs:\n {0}".format("\n".join(failed_entities))
        else:
        ...
        result_value = u"false"
        output_message = u"Error executing action 'Enrich URL'. Reason: {0}".format(e)
