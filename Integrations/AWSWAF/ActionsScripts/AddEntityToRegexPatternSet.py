...
SCRIPT_NAME = "AddEntityToRegexPatternSet"
...
def main():
    ...
    waf_regex_sets = []  # list of Regex Pattern Set data models representing IP Sets in AWS WAF
    status = EXECUTION_STATE_COMPLETED

    regex_list = []  # list of regexes to add to Regex Pattern sets in AWS WAF, either with Domain/IP regex pattern or not.
    ...
    try:
        ...
        for entity in siemplify.target_entities:  # process entities
            ...
            siemplify.LOGGER.info(f"Finished processing entity {entity.identifier} to regex {regex}")
        ...
        for regex_set in existing_regex_sets:  # output message for each Regex Pattern Set block
            if successful_regex_pattern_sets.get(regex_set.scoped_name):
                ...
                output_message += "\n Successfully added the following entity patterns to the {} Regex Pattern Set {} in AWS WAF: \n {}".format(
        ...
        if not successful_regex_pattern_sets:
            output_message += "\n No regexes were added to the provided Regex Pattern sets."
        ...
        siemplify.LOGGER.error(f"Error executing action 'Add Entity To Regex Pattern Set'. Reason: {error}")
        siemplify.LOGGER.exception(error)
        status = EXECUTION_STATE_FAILED
        output_message = f"Error executing action 'Add Entity To Regex Pattern Set'. Reason: {error}"
