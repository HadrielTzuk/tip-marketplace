...
def main():
    ...
    try:
        ...
        waf_client.test_connectivity()
        status = EXECUTION_STATE_COMPLETED
        output_message = "Successfully connected to the AWS WAF server with the provided connection parameters!"
        result_value = "true"

    except Exception as e:
        siemplify.LOGGER.error("Failed to connect to the AWS WAF server! Error is {}".format(e))
        siemplify.LOGGER.exception(e)
        status = EXECUTION_STATE_FAILED
        result_value = "false"
        output_message = "Failed to connect to the AWS WAF server! Error is {}".format(e)
