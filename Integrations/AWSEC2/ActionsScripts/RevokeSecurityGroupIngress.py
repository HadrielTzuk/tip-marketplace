...
from consts import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, REVOKE_SECURITY_GROUP_INGRESS, ALL_IP_PROTOCOLS, IP_PROTOCOLS_MAPPER
...
def main():
    ...
    siemplify.script_name = "{} - {}".format(INTEGRATION_NAME, REVOKE_SECURITY_GROUP_INGRESS)
    ...
    try:
        ...
        for security_group_id in security_group_ids:
            try:
                ...
                manager.revoke_security_group_ingress(
                ...
                siemplify.LOGGER.error(f"Failed to revoke ingress rule of security group with id {security_group_id}")
                ...
                siemplify.LOGGER.exception(error)
                siemplify.LOGGER.error(f"Failed to revoke ingress rule for security group with id {security_group_id}")
                failed_revokes.append(security_group_id)

        if submitted_security_group_ids:
            # Check if ingress rule was removed from security group
            ...
            for security_group_id in submitted_security_group_ids:
                ...
                for ingress_rule in security_group_ingress_rules:
                    # Check if ingress rule exists in security group after revoke
                    if ingress_rule.contains(revoked_security_group_ip_permission):
                        siemplify.LOGGER.info(f"Ingress rule found to exist in security group {security_group_id} after revoke")
                        failed_revokes.append(security_group_id)
                        rule_revoked = False
                        break
        ...
        if successful_revokes:
            output_message += "Successfully revoked the specified ingress rule from the following security groups:  {}. \n\n".format(
                ", ".join(successful_revokes)
            )
            result_value = True

        if invalid_revokes:
            output_message += "Failed to revoke the specified ingress rule from the following security groups:  {}. Reason: " \
        ...
        if failed_revokes:
            output_message += "Failed to revoke the specified ingress rule from the following security groups:  {}".format(
                ", ".join(failed_revokes))
            result_value = False

    except Exception as error:
        siemplify.LOGGER.error(f"Error executing action '{REVOKE_SECURITY_GROUP_INGRESS}'. Reason: {error}")
        siemplify.LOGGER.exception(error)
        status = EXECUTION_STATE_FAILED
        result_value = False
        output_message = f"Error executing action '{REVOKE_SECURITY_GROUP_INGRESS}'. Reason: {error}"
