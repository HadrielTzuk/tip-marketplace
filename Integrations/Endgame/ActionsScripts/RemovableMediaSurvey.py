...
SCRIPT_NAME = u"Removable Media Survey"
INVESTIGATION_NAME = u"Siemplify Removable Media Survey"
...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"removableMediaSurveyRequest", endpoint.core_os)
        ...
        if successful_entities:
            output_message += u"Successfully initiated removable media survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating removable media survey on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
        ...
        for entity_identifier, investigation_id in investigation_ids.items():
            try:
                results = []
                siemplify.LOGGER.info(u"Collecting removable media information for {}".format(entity_identifier))
                # Collect the results of the investigation
                investigation_results = endgame_manager.retrieve_investigation_results(
                    investigation_id,
                    scope=u"removable_media",
                    limit=results_limit
                )
                ...
                if results:
                    ...
                    siemplify.result.add_data_table(
                        u"Information about removable media from {}".format(entity_identifier),
        ...
        if successful_entities:
            output_message = u"Successfully completed removable media survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"Removable media survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nRemovable media survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running removable media survey on the following entities:\n   {}".format(
...
def result_to_csv(result):
    ...
    return {
        ...
        u"Serial Number": result.get(u"serial_number"),
        u"Vendor ID": result.get(u"vendor_id"),
        u"Connected": result.get(u"is_connected"),
        u"Last Connection Time": result.get(u"last_connect_time")
    }
