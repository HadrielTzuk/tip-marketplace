...
from EndgameManager import EndgameManager, EndgameNotFoundError, RELEASE_REQUESTED
...
TASK_NAME = u"hostIsolationReleaseRequest"
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            ...
            try:
                ...
                endgame_manager.initialize_isolation_task(task_id, sensor_ids, isolate=False)

                if endgame_manager.get_endpoint_by_id(endpoint.id).isolation_request_status == RELEASE_REQUESTED:
                    siemplify.LOGGER.info(
                        u"Successfully initiated release of {0} endpoint with Endgame".format(entity.identifier))
                    successful_entities.append(entity)

                    if is_insight:
                        siemplify.add_entity_insight(
                            entity,
                            u"Host release was initiated using Endgame",
                            triggered_by=INTEGRATION_NAME
                        )

                else:
                    siemplify.LOGGER.info(u"Failed to release isolation of {0}.".format(entity.identifier))
        ...
        if successful_entities:
            output_message += u"Successfully initiated release of the following entities:\n   {}".format(
                u"\n   ".join([entity.identifier for entity in successful_entities])
            )
        else:
            output_message += u"No entities were released."

        if missing_entities:
            output_message += u"The following entities didn't match an any endpoint and were skipped:\n   {}".format(
                u"\n   ".join([entity.identifier for entity in missing_entities])
            )

        if no_sensors_entities:
            output_message += u"No sensors were found for the following entities and therefore were skipped:\n   {}".format(
                u"\n   ".join([entity.identifier for entity in no_sensors_entities])
            )

        if failed_entities:
            output_message += u"Failed to initiate release of the following entities:\n   {}".format(
                u"\n   ".join([entity.identifier for entity in failed_entities])
            )

        if error_entities:
            output_message += u"Error occurred while initiating release of the following entities:\n   {}".format(
