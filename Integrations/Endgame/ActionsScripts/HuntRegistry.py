...
from EndgameManager import EndgameManager, EndgameNotFoundError, IOC_REGISTRY_SEARCH
...
SCRIPT_NAME = u'Hunt Registry'
INVESTIGATIOM_NAME = u"Siemplify Registry Hunt Investigation API"
...
def main():
    ...
    hive = extract_action_param(siemplify, param_name=u"Hive", is_mandatory=False,
                                        input_type=unicode, default_value=u"ALL", print_value=True)

    keys = extract_action_param(siemplify, param_name=u"Keys", is_mandatory=False,
                                        input_type=unicode, print_value=True)
    keys = [key.strip() for key in keys.split(u",")] if keys else []

    min_size = extract_action_param(siemplify, param_name=u"Min Size", is_mandatory=False,
                                        input_type=unicode, print_value=True)

    max_size = extract_action_param(siemplify, param_name=u"Max Size", is_mandatory=False,
                                        input_type=unicode, print_value=True)
    ...
    try:
        ...
        if successful_entities:
            siemplify.LOGGER.info(u"Initiating registry hunt task on the following entities: {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            ))

            task_id = endgame_manager.get_task_id(u"iocSearchRequest", core_os)
            task = endgame_manager.create_ioc_registry_search_task(
                hive=hive,
                key=keys,
                min_size=min_size,
                max_size=max_size
            ...
            if successful_entities:
                output_message += u"Successfully initiated registry hunt on the following entities:\n   {}".format(
                    u"\n   ".join([entity for entity in successful_entities])
                )
            ...
            if failed_entities:
                output_message += u"\n\nError occurred while initiating registry hunt on the following entities:\n   {}".format(
