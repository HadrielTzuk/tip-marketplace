...
SCRIPT_NAME = u"User Sessions Survey"
INVESTIGATION_NAME = u"Siemplify User Sessions Survey"
...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"userSessionsSurveyRequest", endpoint.core_os)
        ...
        if successful_entities:
            output_message += u"Successfully initiated user sessions survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating user sessions survey on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
        ...
        for entity_identifier, investigation_id in investigation_ids.items():
            try:
                results = []
                siemplify.LOGGER.info(u"Collecting user sessions information for {}".format(entity_identifier))
                # Collect the results of the investigation
                investigation_results = endgame_manager.retrieve_investigation_results(
                    investigation_id,
                    scope=u"user_sessions",
                    limit=results_limit
                )
                ...
                if results:
                    if endgame_manager.get_investigation_core_os(investigation_id) == WINDOWS_OS:
                        ...
                        siemplify.result.add_data_table(
                            u"User Sessions on {}".format(entity_identifier),
        ...
        if successful_entities:
            output_message = u"Successfully completed user sessions survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"User sessions survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nUser sessions survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running user sessions survey on the following entities:\n   {}".format(
...
def result_to_csv_windows(result):
    ...
    return {
        u"Username": result.get(u"username"),
        u"Domain": result.get(u"domain"),
        u"Logon Type": result.get(u"logon_type"),
...
def result_to_csv_unix(result):
    ...
    return {
        u"Username": result.get(u"username"),
        u"UID": result.get(u"uid"),
        u"GID": result.get(u"gid"),
        u"Creation Time": result.get(u"started"),
        u"Terminal": result.get(u"terminal"),
        u"Shell": result.get(u"shell"),
        u"Session Amount": result.get(u"session_count")
    }
