...
SCRIPT_NAME = u"Network Survey"
INVESTIGATION_NAME = u"Siemplify Network Survey"
SUPPORTED_ENTITIES = [EntityTypes.ADDRESS, EntityTypes.HOSTNAME]
SUPPORTED_OSES = [u"windows", u"linux", u"solaris"]
...
def main():
    ...
    include_arp = extract_action_param(siemplify, param_name=u"Include ARP Table Information",
                                                     is_mandatory=False, input_type=bool, print_value=True,
                                                     default_value=False)

    include_dns = extract_action_param(siemplify, param_name=u"Include DNS Cache Information",
                                                     is_mandatory=False, input_type=bool, print_value=True,
                                                     default_value=False)

    include_net_bios = extract_action_param(siemplify, param_name=u"Include Net Bios Information",
                                                     is_mandatory=False, input_type=bool, print_value=True,
                                                     default_value=False)

    include_route_entries = extract_action_param(siemplify, param_name=u"Include Route Entries Information",
                                                     is_mandatory=False, input_type=bool, print_value=True,
                                                     default_value=False)
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"systemNetworkSurveyRequest", endpoint.core_os)
                    task = endgame_manager.create_network_survey_task(
                        extended_survey=(include_arp or include_dns or include_net_bios or include_route_entries)
                    )
                    ...
                    investigation_id = endgame_manager.create_investigation(
                        ...
                        tasks={task_description_id: task},
        ...
        if successful_entities:
            output_message += u"Successfully initiated network survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating network survey on the following entities:\n   {}".format(
    ...
    try:
        ...
        for entity_identifier, investigation_id in investigation_ids.items():
            try:
                ...
                connections = endgame_manager.retrieve_investigation_scope_results(
                    investigation_id,
                    scope=u"connections",
                    limit=results_limit
                )
                ...
                if connections:
                    ...
                    siemplify.result.add_data_table(
                        u"Information about connections from {}".format(entity_identifier),
                        construct_csv([connections_to_csv(result) for result in connections])
                    )

                if include_arp:
                    siemplify.LOGGER.info(u"Collecting ARP entries for {}".format(entity_identifier))
                    arp_table = endgame_manager.retrieve_investigation_scope_results(
                        investigation_id,
                        scope=u"arp_entries",
                        limit=results_limit
                    )
                    ...
                    json_results[entity_identifier].update({
                        u"arp_table": connections
                    })

                    if arp_table:
                        # Add table for each entity
                        siemplify.result.add_data_table(
                            u"ARP Table from {}".format(entity_identifier),
                            construct_csv([arp_to_csv(result) for result in arp_table])
                        )

                if include_dns:
                    siemplify.LOGGER.info(u"Collecting DNS Cache records for {}".format(entity_identifier))
                    dns_cache_records = endgame_manager.retrieve_investigation_scope_results(
                        investigation_id,
                        scope=u"dns_cache_records",
                        limit=results_limit
                    )

                    json_results[entity_identifier].update({
                        u"dns_cache": dns_cache_records
                    })

                    if dns_cache_records:
                        # Add table for each entity
                        siemplify.result.add_data_table(
                            u"DNS cache from {}".format(entity_identifier),
                            construct_csv([dns_to_csv(result) for result in dns_cache_records])
                        )

                if include_net_bios:
                    siemplify.LOGGER.info(u"Collecting NetBIOS info for {}".format(entity_identifier))
                    netbios_info = endgame_manager.retrieve_investigation_scope_results(
                        investigation_id,
                        scope=u"netbios_neighbors",
                        limit=results_limit
                    )

                    json_results[entity_identifier].update({
                        u"netbios_info": netbios_info
                    })

                    if netbios_info:
                        # Add table for each entity
                        siemplify.result.add_data_table(
                            u"NetBIOS information from {}".format(entity_identifier),
                            construct_csv([net_bios_to_csv(result) for result in netbios_info])
                        )

                if include_route_entries:
                    siemplify.LOGGER.info(u"Collecting route table entires for {}".format(entity_identifier))
                    route_entries = endgame_manager.retrieve_investigation_scope_results(
                        investigation_id,
                        scope=u"route_entries",
                        limit=results_limit
                    )

                    json_results[entity_identifier].update({
                        u"route_table": route_entries
                    })

                    if route_entries:
                        # Add table for each entity
                        siemplify.result.add_data_table(
                            u"Route Table from {}".format(entity_identifier),
                            construct_csv([route_table_to_csv(result) for result in route_entries])
                        )
        ...
        if successful_entities:
            output_message = u"Successfully completed network survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"Network survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nNetwork survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running network survey on the following entities:\n   {}".format(
...
def connections_to_csv(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Process Name": result.get(u"name"),
        u"Process ID": result.get(u"pid"),
        u"State": result.get(u"connection_status"),
        u"Local Address": result.get(u"local_address"),
        u"Local Port": result.get(u"local_port"),
        u"Remote Address": result.get(u"remote_address"),
        u"Remote Port": result.get(u"remote_port"),
        u"Protocol": result.get(u"protocol"),
        u"Creation time": result.get(u"create_time")
    }


def arp_to_csv(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"IP Address": result.get(u"layer3_address"),
        u"Physical Address": result.get(u"layer2_address"),
        u"Interface Name": result.get(u"interface_name"),
        u"Protocol": result.get(u"layer3_family")
    }


def dns_to_csv(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Name": result.get(u"name"),
        u"Record Type": result.get(u"dns_record_type"),
        u"TTL": result.get(u"ttl"),
        u"Query Error": result.get(u"query_error")
    }


def net_bios_to_csv(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Name": result.get(u"name"),
        u"Major Version": result.get(u"version_major"),
        u"Minor Version": result.get(u"version_minor"),
        u"Platform": result.get(u"platform")
    }


def route_table_to_csv(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Destination": result.get(u"destination"),
        u"Gateway": result.get(u"gateway"),
        u"Netmask": result.get(u"netmask"),
        u"Protocol": result.get(u"family"),
        u"Interface Name": result.get(u"interface_name")
