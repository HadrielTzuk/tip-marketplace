...
SCRIPT_NAME = u"Process Survey"
INVESTIGATION_NAME = u"Siemplify Process Survey"
...
def main():
    ...
    only_suspicious_processes = extract_action_param(siemplify, param_name=u"Return Only Suspicious Processes",
                                                     is_mandatory=False, input_type=bool, print_value=True,
                                                     default_value=False)

    collect_process_threads = extract_action_param(siemplify, param_name=u"Collect Process Threads",
                                                   is_mandatory=False, input_type=bool, print_value=True,
                                                   default_value=False)

    detect_malware = extract_action_param(siemplify, param_name=u"Detect Malware With MalwareScore (Windows Only)",
                                          is_mandatory=False, input_type=bool, print_value=True,
                                          default_value=False)

    detect_fileless_attacks = extract_action_param(siemplify, param_name=u"Detect Fileless Attacks (Windows Only)",
                                                   is_mandatory=False, input_type=bool, print_value=True,
                                                   default_value=False)
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"processSurveyRequest", endpoint.core_os)

                    if endpoint.core_os.lower() == WINDOWS_OS:
                        task = endgame_manager.create_process_survey_task(
                            only_suspicious_processes=only_suspicious_processes,
                            collect_process_threads=collect_process_threads,
                            detect_malware=detect_malware,
                            detect_fileless_attacks=detect_fileless_attacks
                        )

                    else:
                        siemplify.LOGGER.info(u"Endpoint {} is not windows machine. Detect Malware & Detect Fileless "
                                              u"Attacks are not supported and will be ignored.".format(
                            entity.identifier)
                        )
                        task = endgame_manager.create_process_survey_task(
                            only_suspicious_processes=only_suspicious_processes,
        ...
        if successful_entities:
            output_message += u"Successfully initiated process survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating process survey on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
        ...
        for entity_identifier, investigation_id in investigation_ids.items():
            try:
                results = []
                siemplify.LOGGER.info(u"Collecting process information for {}".format(entity_identifier))
                ...
                if results:
                    if endgame_manager.get_investigation_core_os(investigation_id) == WINDOWS_OS:
                        ...
                        siemplify.result.add_data_table(
                            u"Processes on {}".format(entity_identifier),
                            construct_csv([result_to_csv_windows(result) for result in results])
                        )
                    else:
                        # Add table for each entity
                        siemplify.result.add_data_table(
                            u"Processes on {}".format(entity_identifier),
                            construct_csv([result_to_csv_unix(result) for result in results])
                        )
        ...
        if successful_entities:
            output_message = u"Successfully completed process survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"Process survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nProcess survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running process survey on the following entities:\n   {}".format(
...
def result_to_csv_windows(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Process Name": result.get(u"name"),
        u"PID": result.get(u"pid"),
        u"CMD Line": result.get(u"cmdline"),
        u"Parent Process Name": result.get(u"parent_name"),
        u"PPID": result.get(u"ppid"),
        u"SID": result.get(u"sid"),
        u"Threads": len(result.get(u"threads")),
        u"Signature Signer": result.get(u"modules", {}).get(u"signature_signer"),
        u"Signature Status": result.get(u"signature_status"),
        u"MalwareScore": result.get(u"malware_classification", {}).get(u"score")
    }


def result_to_csv_unix(result):
    # TODO: Create a datamodel for this type of result and move this method there
    return {
        u"Process Name": result.get(u"name"),
        u"PID": result.get(u"pid"),
        u"CMD Line": result.get(u"cmdline"),
        u"Path": result.get(u"exe"),
        u"Parent Process Name": result.get(u"parent_name"),
        u"PPID": result.get(u"ppid"),
        u"User": result.get(u"user"),
        u"Suspicious Name": result.get(u"name_suspicious"),
        u"Threads": result.get(u"num_threads"),
        u"Creation Time": result.get(u"create_time"),
        u"Up Time": result.get(u"up_time")
    }
