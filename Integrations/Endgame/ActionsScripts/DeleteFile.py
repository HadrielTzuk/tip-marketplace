...
SCRIPT_NAME = u"Delete File"
INVESTIGATION_NAME = u"Siemplify Delete File"
...
def main():
    ...
    full_file_path = extract_action_param(siemplify, param_name=u"File Path", is_mandatory=True,
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id("deleteFileRequest", endpoint.core_os.lower())
                    task = endgame_manager.create_delete_file_task(
        ...
        if successful_entities:
            output_message += u"Successfully initiated file deletion on the following entities:\n   {}".format(
            ...
            if failed_entities:
                output_message += u"\n\nError occurred while initiating file deletion on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
                ...
                if endgame_manager.is_task_failed(bulk_task_id):
                    try:
                        ...
                        siemplify.LOGGER.error(
                            u"Delete file task failed on entity {}. Local message: {}. System message: {}".format(
                                entity_identifier,
                                local_msg,
                                system_msg)
                        )
                    ...
                    siemplify.LOGGER.info(
                        u"File deletion task {} of entity {} completed successfully.".format(
                            bulk_task_id,
                            entity_identifier)
                    )
        ...
        output_message = u"Successfully deleted files from the following entities:\n   {}".format(
            u"\n   ".join([entity for entity in successful_entities])
        )

        if failed_entities:
            output_message += u"\n\nFile deletion task has failed on the following entities:\n   {}".format(
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running file delete task on the following entities:\n   {}".format(
