...
SCRIPT_NAME = u"Drivers Survey"
INVESTIGATION_NAME = u"Siemplify Drivers Survey"
...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"kernelModulesSurveyRequest", endpoint.core_os)
        ...
        if successful_entities:
            output_message += u"Successfully initiated drivers survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating drivers survey on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
            ...
            try:
                ...
                if results:
                    ...
                    siemplify.result.add_data_table(
                        u"Information about drivers from {}".format(entity_identifier),
        ...
        if successful_entities:
            output_message = u"Successfully completed drivers survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"Drivers survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nDrivers survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running drivers survey on the following entities:\n   {}".format(
...
def result_to_csv(result):
    ...
    return {
        u"Name": result.get(u"driver_basename"),
        u"File Path": result.get(u"driver_filename"),
        u"Description": result.get(u"driver_description"),
        u"Date Modified": result.get(u"date_modified")
    }
