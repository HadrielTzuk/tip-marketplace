...
from EndgameManager import EndgameManager, EndgameNotFoundError, IOC_PROCESS_SEARCH
...
SCRIPT_NAME = u'Hunt Process'
INVESTIGATIOM_NAME = u"Siemplify Process Hunt Investigation API"
...
def main():
    ...
    find_process = extract_action_param(siemplify, param_name=u"Process Name", is_mandatory=False,
                                     input_type=unicode, print_value=True)

    siemplify.LOGGER.info(u"----------------- Main - Started -----------------")

    if not (md5_hashes or sha1_hashes or sha256_hashes or find_process):
        siemplify.end(
            u"ERROR: You must enter at least one search field.",
            u'false',
    ...
    try:
        ...
        if successful_entities:
            siemplify.LOGGER.info(u"Initiating process hunt task on the following entities: {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            ))

            task_id = endgame_manager.get_task_id(u"iocSearchRequest", core_os)
            task = endgame_manager.create_ioc_process_search_task(
            ...
            if successful_entities:
                output_message += u"Successfully initiated process hunt on the following entities:\n   {}".format(
                    u"\n   ".join([entity for entity in successful_entities])
                )
            ...
            if failed_entities:
                output_message += u"\n\nError occurred while initiating process hunt on the following entities:\n   {}".format(
