...
SCRIPT_NAME = u"Firewall Survey"
INVESTIGATION_NAME = u"Siemplify Firewall Survey"
...
def main():
    ...
    try:
        ...
        for entity in siemplify.target_entities:
            try:
                ...
                if endpoint.sensors:
                    ...
                    task_description_id = endgame_manager.get_task_id(u"firewallSurveyRequest", endpoint.core_os)
        ...
        if successful_entities:
            output_message += u"Successfully initiated firewall survey on the following entities:\n   {}".format(
        ...
        if failed_entities:
            output_message += u"\n\nError occurred while initiating firewall survey on the following entities:\n   {}".format(
...
def async_action():
    ...
    try:
            ...
            try:
                ...
                if results:
                    ...
                    siemplify.result.add_data_table(
                        u"Firewall rules on {}".format(entity_identifier),
        ...
        if successful_entities:
            output_message = u"Successfully completed firewall survey on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in successful_entities])
            )

        else:
            output_message = u"Firewall survey didn't run on any entities."
            result_value = u"false"

        if failed_entities:
            output_message += u"\n\nFirewall survey has failed on the following entities:\n   {}".format(
                u"\n   ".join([entity for entity in failed_entities])
            )
        ...
        if error_entities:
            output_message += u"\n\nError occurred while running firewall survey on the following entities:\n   {}".format(
...
def result_to_csv(result):
    ...
    return {
        u"Name": result.get(u"rule_name"),
        u"Application Name": result.get(u"application_name"),
        u"Direction": result.get(u"direction"),
        u"Action": result.get(u"action"),
        u"Local Address": result.get(u"local_addresses"),
        u"Local Port": result.get(u"local_ports"),
        u"Remote Address": result.get(u"remote_addresses"),
        u"Remote Port": result.get(u"remote_ports"),
        u"Description": result.get(u"description"),
