    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                msg = 'Successfully added tag {} to {}'.format(tag_name, entity.identifier)
                siemplify.LOGGER.info(msg)
                output_message += '\n\n{}'.format(msg)
            except EndpointNotFoundException:
                failed_entities.append(entity)
                msg = "Action wasn't able to add tag '{0}' to {1}. Reason: Endpoint {1} was not found in McAfee Mvision ePO.".format(
                    tag_name, entity.identifier)
                siemplify.LOGGER.error(msg)
                output_message += '\n\n{}'.format(msg)
            except Exception as e:
                msg = "Action wasn't able to add tag '{0}' to {1}.".format(
                    tag_name, entity.identifier)
                output_message += '\n\n{}'.format(msg)
                failed_entities.append(entity)
                siemplify.LOGGER.error(msg)
        ...
        result_value = False
        output_message = "Action wasn't able to add tag '{0}'. Reason: Tag '{0}' was not found in McAfee Mvision ePO. Please check for any spelling mistakes. In order to get the list of available tags execute action 'List Tags'".format(
            tag_name)
        siemplify.LOGGER.error(output_message)

    except Exception as e:
        output_message = "Error executing action '{}'. Reason: {}".format(
            ADD_TAG_SCRIPT_NAME, e)
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
