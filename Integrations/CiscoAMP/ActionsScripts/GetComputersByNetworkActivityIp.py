        ...
        try:
            if entity.entity_type == EntityTypes.ADDRESS:
                ...
                if computers:
                    ...
                    siemplify.result.add_data_table(u"Computers - {}".format(entity.identifier), csv_output)

                    json_results[entity.identifier] = computers_dict
                    enriched_entities.append(entity)

        except Exception as e:
            errors += "Unable to get computer by ip {0}: \n{1}\n".format(
                entity.identifier, e.message)
            continue

    if enriched_entities:
        entities_names = [entity.identifier for entity in enriched_entities]
        output_message = 'Cisco AMP - Got computers by the following ips\n' + '\n'.join(
            entities_names)
