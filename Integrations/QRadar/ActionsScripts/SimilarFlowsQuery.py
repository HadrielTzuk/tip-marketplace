...
from SiemplifyUtils import convert_dict_to_json_result_dict, construct_csv
...
from constants import INTEGRATION_NAME, SIMILAR_FLOWS_QUERY_SCRIPT_NAME, SIMILAR_FLOWS_TABLE_HEADER, TIME_DELTA, LIMIT,\
...
def start_operation(siemplify, manager, suitable_entities):
    ...
    flows_limit_to_fetch = extract_action_param(siemplify, param_name='Flows Limit To Fetch', print_value=True,
                                                is_mandatory=True, input_type=int, default_value=LIMIT)
    ...
    for entity in suitable_entities:
        try:
            search_id = manager.search_for_items(entity=get_entity_original_identifier(entity),
                                                 entity_type=entity.entity_type, action_type=FLOWS_DATA_TYPE_IDENTIFIER,
                                                 time_delta=time_delta,
                                                 limit=flows_limit_to_fetch, fields=fields_to_display,
                                                 source_address_field=source_address_field,
    ...
    if successful_entities:
        ...
        output_message = "Action didnt complete successfully because {0}".format(
            invalid_parameter_output_message)
        result_value = False
        status = EXECUTION_STATE_COMPLETED

    else:
        output_message = "No similar flows were found."
        result_value = False
        status = EXECUTION_STATE_COMPLETED
...
def finish_operation(siemplify, manager, completed_entities, failed_entities, suitable_entities_identifiers):
    ...
    for entity_identifier in suitable_entities_identifiers:
        ...
        try:
            entity_result = manager.get_search_report(search_id=search_id, report_type=FLOWS_DATA_TYPE_IDENTIFIER)
            if entity_result:
                siemplify.result.add_data_table(
                    SIMILAR_FLOWS_TABLE_HEADER.format(entity_identifier),
    ...
    if successful_entities:
        output_message += "Similar flows were found for the following entities:\n{}\n"\
            .format(', '.join(successful_entities))

    if not_found_entities:
        output_message += "The following entities were processed successfully, but no similar flows were found for " \
...
def main(is_first_run):
    ...
    siemplify.script_name = SIMILAR_FLOWS_QUERY_SCRIPT_NAME
