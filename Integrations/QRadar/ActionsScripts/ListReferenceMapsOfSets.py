...
from constants import (
    INTEGRATION_NAME,
    LIST_REFERENCE_MAPS_OF_SETS_SCRIPT_NAME,
    LIMIT
)
...
def main():
    ...
    siemplify.script_name = LIST_REFERENCE_MAPS_OF_SETS_SCRIPT_NAME
    ...
    try:
        ...
        reference_maps = manager.get_reference_maps_of_sets(fields_to_return, filter_condition, results_limit)
        if reference_maps:
            output_message = "Action completed successfully and returned data."
            siemplify.result.add_result_json([ref_map.to_json() for ref_map in reference_maps])
        else:
        ...
        output_message = f'Error executing {LIST_REFERENCE_MAPS_OF_SETS_SCRIPT_NAME}. Reason {e}'
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
