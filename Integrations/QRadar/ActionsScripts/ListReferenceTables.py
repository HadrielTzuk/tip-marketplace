...
from constants import (
    INTEGRATION_NAME,
    LIST_REFERENCE_TABLES_SCRIPT_NAME,
    LIMIT
)
...
def main():
    ...
    siemplify.script_name = LIST_REFERENCE_TABLES_SCRIPT_NAME
    ...
    try:
        ...
        reference_tables = manager.get_reference_tables(fields_to_return, filter_condition, results_limit)
        if reference_tables:
            output_message = "Action completed successfully and returned data."
            siemplify.result.add_result_json([ref_table.to_json() for ref_table in reference_tables])
        else:
        ...
        output_message = f'Error executing {LIST_REFERENCE_TABLES_SCRIPT_NAME}. Reason {e}'
        siemplify.LOGGER.error(output_message)
        siemplify.LOGGER.exception(e)
