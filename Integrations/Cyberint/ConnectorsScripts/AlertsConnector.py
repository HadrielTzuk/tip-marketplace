...
from constants import CONNECTOR_NAME, DEFAULT_LIMIT, DEFAULT_TIME_FRAME, SEVERITIES
...
def main(is_test_run):
    ...
    try:
        ...
        if lowest_severity_to_fetch and lowest_severity_to_fetch.lower() not in list(SEVERITIES.values()):
            raise Exception(f"Invalid value given for Lowest Severity To Fetch parameter. Possible values are: "
                            f"{convert_list_to_comma_string([severity.title() for severity in list(SEVERITIES.values())])}.")

        # Read already existing alerts ids
        existing_ids = read_ids(siemplify)
        siemplify.LOGGER.info(f"Successfully loaded {len(existing_ids)} existing alerts from ids file")

        manager = CyberintManager(api_root=api_root,
                                  api_key=api_key,
        ...
        fetched_alerts = []
        ui_severities = list(SEVERITIES.values())
        api_severities = list(SEVERITIES.keys())

        filtered_alerts = manager.get_alerts(
            existing_ids=existing_ids,
            limit=fetch_limit,
            start_timestamp=last_success_time,
            type_filter=siemplify.whitelist,
            severity_filter=api_severities[ui_severities.index(lowest_severity_to_fetch.lower()):] if
            lowest_severity_to_fetch else api_severities
        )
        ...
        for alert in filtered_alerts:
            try:
                ...
                if not pass_filters(siemplify, whitelist_as_a_blacklist, alert, "type"):
                    continue
        ...
        if not is_test_run:
            ...
            save_timestamp(siemplify=siemplify, alerts=fetched_alerts, timestamp_key="created_date")
