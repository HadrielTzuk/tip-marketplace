...
from SiemplifyUtils import output_handler, extract_domain
from constants import PROVIDER_NAME, REMOVE_SENDERS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME, \
    PARAMETERS_DEFAULT_DELIMITER, CONDITIONS, CORRESPONDING_RULES, DOMAIN_RULES, ASYNC_ACTION_MAX_RETRIES, \
    COMPLETED_STATUS
...
from ExchangeExtensionPackExceptions import ExchangeExtensionPackIncompleteInfoException, \
    ...
    ExchangeExtensionPackGssntlmsspException, ExchangeExtensionPackNotFound
...
def get_rules(manager, rules):
    """
    Get existing rules by rule names
    ...
    existing_rules_names = [existing_rule.name for existing_rule in existing_rules]
    ...
    result_value = {
        ...
        "existing_rules": existing_rules_names
    }

    for existing_rule in existing_rules:
        result_value[existing_rule.name] = existing_rule.items

    output_message = "Found {} rules. Continuing...".format(PARAMETERS_DEFAULT_DELIMITER.join(
        [f'"{rule_name}"' for rule_name in existing_rules_names]
    ))

    return output_message, json.dumps(result_value), status


def remove_items_from_rule(siemplify, manager, rules, valid_items, invalid_items, remove_domains, additional_data):
    """
    ...
    :param remove_domains: {bool} Specifies whether domains of the provided email addresses removed from corresponding rules
    :param additional_data: {dict} Additional data from previous iteration
    :return: {tuple} output_message, result_value, status
    """
    processed_rules = additional_data.get("processed_rules", [])
    existing_rules = additional_data.get("existing_rules", [])
    not_processed_rules = list(set(rules) - set(processed_rules))
    rule_name = not_processed_rules[0]
    existing_items = []

    if rule_name not in existing_rules:
        siemplify.LOGGER.info(f"\"{rule_name}\" rule was not found.")
    else:
        existing_items = additional_data.get(rule_name, [])

        if rule_name in DOMAIN_RULES:
            domains = list(set([extract_domain(email_address) for email_address in valid_items]))
    ...
    if len(rules) == len(processed_rules):
        siemplify.result.add_result_json({
            "success": list(set(existing_items).intersection(valid_items)),
            "didn't_exist": list(set(valid_items) - set(existing_items)),
            "invalid": invalid_items
        })

        result_value = True
        status = EXECUTION_STATE_COMPLETED
        output_message = prepare_output_messages(rules, valid_items, invalid_items, remove_domains)
    else:
...
def main():
    ...
    siemplify.script_name = REMOVE_SENDERS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME
    ...
    rule_name = extract_action_param(siemplify, param_name="Rule to remove Senders from", is_mandatory=True,
                                     print_value=True)
    remove_domains = extract_action_param(siemplify, param_name="Should remove senders' domains from the corresponding"
                                                                " Domains List rule as well?",
                                          input_type=bool, print_value=True)
    ...
    rules = [rule_name, CORRESPONDING_RULES[rule_name]] if remove_domains else [rule_name]
    ...
    try:
        ...
        if additional_data.get("get_rules") != COMPLETED_STATUS:
            ...
            output_message, result_value, status = remove_items_from_rule(siemplify, manager, rules,
                                                                          valid_email_addresses, invalid_email_addresses,
                                                                          remove_domains, additional_data)
        ...
        siemplify.LOGGER.error(f"General error performing action "
                               f"{REMOVE_SENDERS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME}")
        siemplify.LOGGER.exception(e)
        output_message = f"Error performing \"{REMOVE_SENDERS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME}\" " \
                         f"action: {e}"
...
def prepare_output_messages(rules, valid_email_addresses, invalid_email_addresses, remove_domains=None):
    """
