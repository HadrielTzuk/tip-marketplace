...
from constants import PROVIDER_NAME, REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME, \
    PARAMETERS_DEFAULT_DELIMITER, CONDITIONS, ASYNC_ACTION_MAX_RETRIES, COMPLETED_STATUS
...
def get_rules(manager, rule_name):
    ...
    result_value = json.dumps({
        ...
        existing_rule.name: existing_rule.items
    })
...
def remove_items_from_rule(siemplify, manager, rule_name, valid_domains, invalid_domains, existing_domains):
    """
...
def main():
    ...
    siemplify.script_name = REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME
    ...
    rule_name = extract_action_param(siemplify, param_name="Rule to remove Domains from", is_mandatory=True,
                                     print_value=True)
    ...
    try:
        ...
        if additional_data.get("get_rules") != COMPLETED_STATUS:
            ...
            output_message, result_value, status = remove_items_from_rule(siemplify, manager, rule_name, valid_domains,
                                                                          invalid_domains,
        ...
        output_message = f"Error performing \"{REMOVE_DOMAINS_FROM_EXCHANGE_SIEMPLIFY_MAIL_FLOW_RULES_SCRIPT_NAME}\" " \
                         f"action: {e}"
