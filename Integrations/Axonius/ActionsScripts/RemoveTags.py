...
def main():
    ...
    siemplify.script_name = "{} - {}".format(INTEGRATION_IDENTIFIER, REMOVE_TAGS_SCRIPT_NAME)
    ...
    try:
            ...
            try:
                ...
                try:
                    ...
                    if found_entities_to_process.keys():
                        siemplify.LOGGER.info("Removing tags from entities {}".format(', '.join(found_entities_to_process.keys())))
                        manager.remove_tags_from_users(internal_axonius_ids=list(set(found_entities_to_process.values())), tags=tags)
                    ...
                    siemplify.LOGGER.error("Failed to remove tags from entities: {}".format(', '.join(found_entities_to_process.keys())))
        ...
        if any(devices.values()):
            ...
            try:
                ...
                try:
                    ...
                    if found_entities_to_process.keys():
                        siemplify.LOGGER.info("Removing tags from entities {}".format(', '.join(found_entities_to_process.keys())))
                        manager.remove_tags_from_devices(internal_axonius_ids=list(set(found_entities_to_process.values())), tags=tags)
        ...
        output_message = f'Error execution action \"{REMOVE_TAGS_SCRIPT_NAME}\". Reason: {error}'
        result_value = False
