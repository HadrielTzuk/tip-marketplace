...
def main():
    ...
    siemplify.script_name = QUERY_NET_HOST_ACTION
    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                events = rsa_manager.get_events_for_domain(entity.identifier, hours_backwards)
                if events:
                    ...
                    siemplify.result.add_entity_attachment(entity.identifier,
                                                           ...
                                                           rsa_manager.get_pcap_for_domain(entity.identifier,
                                                                                           hours_backwards))
        ...
        if failed_entities:
            output_message += 'Events were not found for the following hostnames: {}.'.format(', '.join(failed_entities))

        if entities_with_result:
            output_message += 'Successfully found events in RSA NetWitness for the following hostnames: {}.'.\
                format(', '.join(entities_with_result))
        else:
        ...
        output_message = 'Error executing action {}. Reason: {}'.format(QUERY_NET_HOST_ACTION, e)
