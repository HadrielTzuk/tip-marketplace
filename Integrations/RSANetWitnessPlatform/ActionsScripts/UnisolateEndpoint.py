...
def main():
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            if entity_object:
                try:
                    rsa_manager.unisolate_endpoint(agent_id=entity_object.agent_id, service_id=service_object.id,
                                                   comment=comment)
        ...
        if successful_entities:
            output_messages.append('Successfully requested unisolation for the following endpoints from RSA Netwitness:'
                                   ' {}'.format("\n".join([entity.identifier for entity in successful_entities])))

        if failed_entities:
            result_value = False
            output_messages.append("Action was not able to request unisolation for the following endpoints from "
        ...
        if not suitable_entities:
            output_message = "No entities were unisolated."
            result_value = False

    except Exception as e:
        output_message = "Error executing action \"Unisolate Endpoint\". Reason: {}".format(e)
        result_value = False
