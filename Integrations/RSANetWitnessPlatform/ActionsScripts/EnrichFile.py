...
def main():
    ...
    suitable_entities = [entity for entity in siemplify.target_entities if entity.entity_type ==
                         EntityTypes.FILENAME or entity.entity_type == EntityTypes.FILEHASH]
    ...
    try:
        ...
        for entity in suitable_entities:
            ...
            try:
                if entity.entity_type == EntityTypes.FILENAME:
                    entity_object = rsa_manager.search_for_filename(service_id=service_object.id, value=entity.identifier)
                else:
                    entity_object = rsa_manager.search_for_filehash(service_id=service_object.id, value=entity.identifier)

                if entity_object:
                    enrichment_data = entity_object.to_enrichment_data(prefix=ENRICHMENT_PREFIX)
                    entity.additional_properties.update(enrichment_data)
                    entity.is_enriched = True
                    if risk_score_threshold and entity_object.global_risk_score > risk_score_threshold:
                        entity.is_suspicious = True

                    # JSON result
                    json_results[entity.identifier] = entity_object.to_json()
                    siemplify.LOGGER.info(
                        'Successfully enriched the following file from RSA NetWitness: {}'.format(entity.identifier))
        ...
        if failed_entities:
            output_message += "\n\n Action was not able to enrich the following files from RSA NetWitness: {}"\
        ...
        output_message = "Error executing action \"Enrich File\". Reason: {}".format(e)
        result_value = False
