...
def main():
    ...
    siemplify.script_name = QUERY_NET_IP_ACTION
    ...
    try:
        ...
        for entity in suitable_entities:
            try:
                ...
                events = rsa_manager.get_events_for_ip(entity.identifier, hours_backwards)
                if events:
                    ...
                    siemplify.result.add_entity_attachment(entity.identifier,
                                                           ...
                                                           rsa_manager.get_pcap_for_ip(entity.identifier,
                                                                                       hours_backwards))
        ...
        if failed_entities:
            output_message += 'Events were not found for the following IPs: {}.'.format(', '.join(failed_entities))

        if entities_with_result:
            output_message += 'Successfully found events in RSA NetWitness for the following IPs: {}.'.format(
        ...
        output_message = 'Error executing action {}. Reason: {}'.format(QUERY_NET_IP_ACTION, e)
