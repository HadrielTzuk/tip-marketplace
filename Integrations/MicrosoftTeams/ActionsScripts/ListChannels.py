...
def main():
    ...
    siemplify.script_name = LIST_CHANNELS_ACTION
    ...
    try:
        ...
        max_channels_to_return = extract_action_param(siemplify, input_type=int, param_name="Max Channels To Return", print_value=True, is_mandatory=False)
 
        if max_channels_to_return is not None and max_channels_to_return < 1:
            raise Exception("\"Max Channels To Return\" must be greater than 0.")
         
        mtm = MicrosoftTeamsManager(client_id, secret_id, tenant, token, redirect_url)
        channels = mtm.list_channels(team_name=team_name,max_channels_to_return=max_channels_to_return) or []
        result_value = json.dumps(channels)
        
        if channels:
            output_message = "Successfully get channels details in {0} team.".format(team_name)
        else:
        ...
        output_message = f'Error executing action {LIST_CHANNELS_ACTION}. Reason: {e}'
