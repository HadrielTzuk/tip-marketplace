...
from MicrosoftConstants import INTEGRATION_NAME, INTEGRATION_DISPLAY_NAME, CREATE_CHANNEL_ACTION
...
def main():
    ...
    siemplify.script_name = CREATE_CHANNEL_ACTION
    ...
    channel_type = extract_action_param(siemplify, param_name="Channel Type", is_mandatory=True, print_value=True)
    ...
    try:
        ...
        channel = manager.create_channel(team_id, channel_name, channel_type, description)
        siemplify.result.add_result_json(channel.to_json())
        output_message = f"Successfully created channel \"{channel_name}\" in team \"{team_name}\" " \
                         f"in {INTEGRATION_DISPLAY_NAME}."
    except Exception as e:
        output_message = f"Error executing action {CREATE_CHANNEL_ACTION}. Reason: {e}"
        result_value = False
